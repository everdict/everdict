...
export const agentPlane = {
  ...
  // Try-drive (agent-automation B3) — shadow-run a saved agent or a draft against a (replayed) platform event.
  tryAgent: <T>(auth: AuthContext, body: unknown) =>
    call<T>(auth, '/agent/agents/try', { method: 'POST', body: JSON.stringify(body) }),
  ...
  // The write-tool approvals a background (discussion) turn is waiting on — the watch-mode panel polls it and raises the approval prompt.
  ...
  // Soft interrupt (ESC): abort only the in-flight step. With `message`, the server queues it ATOMICALLY
  // (liveness checked first — a redirect racing a finishing turn queues nothing and 404s, so the caller falls
  // back to a normal send) and the turn continues REDIRECTED; bare it ends "interrupted". Distinct from
  // stopTurn (whole-turn abort).
  interruptTurn: <T>(auth: AuthContext, id: string, message?: string) =>
