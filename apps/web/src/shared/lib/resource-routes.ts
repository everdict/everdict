...
// addresses, and two separate things need to agree on the pairing — the redirects that keep old links alive
// (`next.config.ts`) and the sidebar's "which row is active" (a detail page belongs to its collection's row).
// When each derived the pairing itself, a detail page simply lit no nav row at all.
//
