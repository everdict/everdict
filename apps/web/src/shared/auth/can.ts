...
export type WebAction = 'runs:read' | 'runs:submit' | 'harnesses:read' | 'harnesses:register'

const PERMS: Record<string, WebAction[]> = {
  viewer: ['runs:read', 'harnesses:read'],
  member: ['runs:read', 'harnesses:read', 'runs:submit'],
  admin: ['runs:read', 'harnesses:read', 'runs:submit', 'harnesses:register'],
}
