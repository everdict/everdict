...
import type { FsUsageView } from '@/entities/workspace-file'
...
// Settings › Files — the workspace filesystem's GOVERNANCE surface: the storage picture (mirrored entirely
// through the service — the user never touches MinIO) plus per-top-level and whole-tree cleanup. Browsing /
// rendering / editing lives on the main Files page; this view is for seeing where the bytes are and reclaiming
// them. Deletes = admin (canManage: whole-tree clear is settings:write; per-entry removal files:write).
  ...
  function runDelete() {
    ...
    startTransition(async () => {
      ...
      if (!res.ok) setError(res.error)
        ...
        const usageRes = await getFsUsageAction()
