...
// Pick base/candidate versions to navigate to the judge diff URL. The diff is computed by the control plane (diffJudgeSpecs).
// `versions` is passed newest-first (semver-desc) — the picker does not re-sort.
export function JudgeDiffPicker({
  id,
  versions,
  base,
  candidate,
  ...
  const t = useTranslations('judgeVersions')
  ...
  return (
    <div className="flex flex-wrap items-end gap-3">
      <div className="min-w-44 space-y-1.5">
        <Label htmlFor="judge-diff-base">base</Label>
        <Combobox id="judge-diff-base" value={b} onChange={setB} options={options} className="w-full" />
      </div>
      <div className="min-w-44 space-y-1.5">
        <Label htmlFor="judge-diff-candidate">candidate</Label>
        <Combobox
          id="judge-diff-candidate"
          value={c}
          onChange={setC}
          options={options}
          className="w-full"
