...
import { deleteJudgeVersionsAction } from '../api/delete-judge'
...
// Delete-judge dialog — pick any subset of versions (or select all = delete the whole judge) and soft-delete them.
// Mount only when open (the parent gates with `{open && …}`) so state initializes fresh each time.
// Single version = check one, several = check several, whole judge = select all — one surface for all three.
export function DeleteJudgeDialog({
  onClose,
  id,
  versions,
  latest,
  ...
  // List-row entry point opens with everything selected (delete-whole-judge intent); the detail header opens with none.
  ...
  const deletesEntire = allSelected // every live version selected → the judge disappears
  const titleId = `delete-judge-${id}`
