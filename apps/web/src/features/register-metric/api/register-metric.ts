...
export interface ValidateMetricResult {
  ok: boolean
  errors?: string[]
...
export async function validateMetricAction(spec: unknown): Promise<ValidateMetricResult> {
  const ctx = await authContext()
  try {
    return await controlPlane.validateMetric<ValidateMetricResult>(ctx, spec)
  } catch (e) {
...
export async function createMetricAction(spec: unknown): Promise<CreateMetricResult> {
  const ctx = await authContext()
  try {
