        ...
        <PageHeader
          ...
          actions={
            <div className="flex items-center gap-2">
              ...
              {versions.length > 1 && (
                <Link
                  href={`/${workspace}/dataset/${encodeURIComponent(dataset.id)}/diff`}
              ...
              {canPublish && (
                <Link
                  href={`/${workspace}/dataset/${encodeURIComponent(dataset.id)}/new-version?v=${encodeURIComponent(dataset.version)}`}
                  className={buttonVariants({ variant: 'secondary', size: 'sm' })}
                >
      ...
      {relation && relation.harnesses.length > 0 && (
        <div className="flex flex-wrap items-center gap-1.5 text-[12px] text-muted-foreground">
          ...
          {relation.harnesses.map((h) => (
            <Link
              key={h}
              href={`/${workspace}/harness/${encodeURIComponent(h)}`}
