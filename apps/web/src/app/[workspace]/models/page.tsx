...
export default async function ModelsPage({ params }: { params: Promise<{ workspace: string }> }) {
      ...
      <PageHeader
        ...
        actions={
          can(principal?.roles, 'models:write') ? (
            <Link href={`/${workspace}/models/new`}>
              <Button size="sm">
      ...
      {error ? (
        ...
        <div className="space-y-2">
          {models.map((m) => (
            <Link
              key={m.id}
              href={`/${workspace}/models/${encodeURIComponent(m.id)}`}
