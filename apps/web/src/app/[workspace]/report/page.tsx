import Link from 'next/link'
import { BarChart3 } from 'lucide-react'
import { getTranslations } from 'next-intl/server'

import {
  casePass,
  scorecardRecordSchema,
  scorecardsSchema,
  trackOf,
  type ScorecardRecord,
} from '@/entities/scorecard'
import { authContext } from '@/shared/auth/principal'
import { controlPlane } from '@/shared/lib/control-plane'
import { Badge } from '@/shared/ui/badge'
import { Callout } from '@/shared/ui/callout'
import { EmptyState } from '@/shared/ui/empty-state'
import { PageHeader } from '@/shared/ui/page-header'
import { SectionHeader } from '@/shared/ui/section-header'
import { StatCard } from '@/shared/ui/stat-card'

export const dynamic = 'force-dynamic'

// Unified desktop + web report: groups different harnesses/benchmarks evaluated through the same CaseResult→Scorecard flow by track,
// showing the authority-based (state > objective > judge) case pass rate on one screen. A single report for the harness/infra-agnostic eval runtime.
const TRACK_ORDER = ['desktop', 'web', 'other'] as const

function pct(pass: number, total: number): string {
  return total > 0 ? `${Math.round((pass / total) * 100)}%` : '—'
}

export default async function ReportPage({ params }: { params: Promise<{ workspace: string }> }) {
  const { workspace } = await params
  const t = await getTranslations('reportPage')
  const ctx = await authContext()
  let records: ScorecardRecord[] = []
  let error: string | undefined
  try {
    records = scorecardsSchema.parse(await controlPlane.listScorecards(ctx))
  } catch (e) {
    error = e instanceof Error ? e.message : String(e)
  }

  // The list gives only summaries (no case results), so for completed ones fetch the full record and compute the authority-based case-pass.
  const listed = records.filter((r) => r.status === 'succeeded')
  const succeeded = await Promise.all(
    listed.map(async (r) => {
      try {
        return scorecardRecordSchema.parse(await controlPlane.getScorecard(ctx, r.id))
      } catch {
        return r
      }
    })
  )
  const byTrack: Record<string, ScorecardRecord[]> = { desktop: [], web: [], other: [] }
  for (const r of succeeded) byTrack[trackOf(r)].push(r)

  let combPass = 0
  let combTotal = 0
  for (const r of succeeded) {
    const cp = casePass(r)
    combPass += cp.pass
    combTotal += cp.total
  }

  return (
    <div className="space-y-7">
      <PageHeader title={t('title')} description={t('description')} />

      {error && <Callout tone="danger">{t('loadError', { error })}</Callout>}

      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3">
        <StatCard
          label={t('overallPassRate')}
          value={pct(combPass, combTotal)}
          hint={t('casesHint', { pass: combPass, total: combTotal })}
          tone={combTotal > 0 && combPass === combTotal ? 'success' : 'primary'}
        />
        {TRACK_ORDER.filter((track) => byTrack[track].length > 0).map((track) => {
          const p = byTrack[track].reduce(
            (acc, r) => {
              const cp = casePass(r)
              return { pass: acc.pass + cp.pass, total: acc.total + cp.total }
            },
            { pass: 0, total: 0 }
          )
          return (
            <StatCard
              key={track}
              label={t(`track.${track}`)}
              value={pct(p.pass, p.total)}
              hint={t('trackHint', { pass: p.pass, total: p.total, count: byTrack[track].length })}
              tone="default"
            />
          )
        })}
      </div>

      {TRACK_ORDER.filter((track) => byTrack[track].length > 0).map((track) => (
        <section key={track} className="space-y-2.5">
          <SectionHeader title={t(`track.${track}`)} />
          <div className="space-y-2">
            {byTrack[track].map((r) => {
              const cp = casePass(r)
              const allPass = cp.total > 0 && cp.pass === cp.total
              return (
                <Link
                  key={r.id}
                  href={`/${workspace}/scorecards/${r.id}`}
                  className="flex flex-wrap items-center justify-between gap-3 rounded-lg border bg-card p-3.5 shadow-raise transition-colors hover:border-border-strong hover:bg-elevated"
                >
                  <span className="min-w-0 truncate font-mono text-[13px] font-[510] text-foreground">
                    {r.dataset.id}
                    <span className="text-faint">@{r.dataset.version}</span>
                    <span className="px-1 text-muted-foreground">→</span>
                    {r.harness.id}
                    <span className="text-faint">@{r.harness.version}</span>
                  </span>
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-[12px] tabular-nums text-muted-foreground">
                      <span className="text-faint">{t('passLabel')}</span> {cp.pass}/{cp.total} (
                      {pct(cp.pass, cp.total)})
                    </span>
                    <Badge tone={allPass ? 'success' : cp.pass > 0 ? 'info' : 'danger'}>
                      {allPass ? t('allPassed') : t('someFailed', { count: cp.total - cp.pass })}
                    </Badge>
                  </div>
                </Link>
              )
            })}
          </div>
        </section>
      ))}

      {succeeded.length === 0 && !error && (
        <EmptyState icon={<BarChart3 />} title={t('emptyTitle')} hint={t('emptyHint')} />
      )}
    </div>
  )
}
