...
export default async function MetricsPage({ params }: { params: Promise<{ workspace: string }> }) {
      ...
      <PageHeader
        ...
        actions={
          can(principal?.roles, 'metrics:write') ? (
            <Link href={`/${workspace}/metrics/new`}>
              <Button size="sm">
      ...
      {error ? (
        ...
        <div className="space-y-2">
          {metrics.map((m) => (
            <Link
              key={m.id}
              href={`/${workspace}/metrics/${encodeURIComponent(m.id)}`}
