...
function actions(): TrayMenuActions & { openApp: ReturnType<typeof vi.fn> } {
  return { openApp: vi.fn(), setAutostart: vi.fn(), unpairRunner: vi.fn(), quit: vi.fn() };
}
...
describe("buildTrayMenuTemplate", () => {
  it("상태행(비활성)/열기/자동시작/종료 를 노출한다 — 미페어 시 해제 항목 없음", () => {
    const t = buildTrayMenuTemplate({ autostart: false, runner: OFF }, actions());
  ...
  it("페어 상태면 '이 기기 러너 연결 해제' 가 나타나고 클릭 시 unpairRunner", () => {
    const a = actions();
    const t = buildTrayMenuTemplate({ autostart: false, runner: IDLE }, a);
  ...
  it("자동시작 체크 상태를 반영하고, 클릭은 반전값으로 setAutostart 를 부른다", () => {
    const a = actions();
    const on = buildTrayMenuTemplate({ autostart: true, runner: OFF }, a);
  ...
  it("열기/종료 클릭이 액션을 호출한다", () => {
    const a = actions();
    const t = buildTrayMenuTemplate({ autostart: false, runner: OFF }, a);
