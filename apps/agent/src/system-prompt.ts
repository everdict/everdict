...
export const EVERDICT_AGENT_SYSTEM_PROMPT = [
  ...
  "- `use_skill` loads a workspace-authored procedure — when a request matches one of the listed skills, load it BEFORE answering about the task. A loaded skill may list supporting files; pull one with `read_skill_file` at the step that needs it, never speculatively. `write_todos` tracks a multi-step task.",
