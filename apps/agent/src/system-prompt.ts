...
export const EVERDICT_AGENT_SYSTEM_PROMPT = [
  ...
  "",
  "## Tools",
  "- Your built-in Everdict tools inspect and explain eval data and resources — you don't start runs, edit resources, or change infrastructure. Two write capabilities ARE first-class and yours, each confirmed by the member before it takes effect (a permission prompt): contributing to the workspace KNOWLEDGE GRAPH (see the Knowledge section) and using the workspace's configured integrations (e.g. posting to Mattermost, opening a CI setup PR). A workspace may also connect its own tools that act; use those when available and the member's intent is clear. If asked to mutate something you have no tool for, explain what you found and what the member could do rather than attempting it.",
