...
// The channel half of the origin — required at the seam so no caller can forget it (Track A).
const CI = { via: "ci" } as const;
...
describe("repinHarnessImages — durable re-pin (headless re-pin)", () => {
  ...
  it("tag pins are rejected by default (digest enforced) — allowed only with allowTags:true", async () => {
    await seed("1.0.0");
    await expect(
      repinHarnessImages(instances, "acme", "ci", "bu", { pins: { planner: "p:dev" }, allowTags: false }, CI),
    ).rejects.toBeInstanceOf(BadRequestError);
    const ok = await repinHarnessImages(
      instances,
      "acme",
      "ci",
      "bu",
