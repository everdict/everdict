...
export interface ExecuteCaseDeps {
  ...
  // Source factory for out-of-job trace collection (collect="control-plane") (@everdict/trace). If unset, surface that collection is impossible.
  buildTraceSource?: (cfg: TraceSourceConfig) => TraceSource;
  // Tenant SecretStore (decrypted values) for re-resolving traceRef.authSecret (name) — the Authorization header for the collection pull.
  secretsFor?: (tenant: string) => Promise<Record<string, string>>;
  sleep?: (ms: number) => Promise<void>; // collection-retry backoff (test-injected, default setTimeout)
