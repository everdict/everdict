...
import { type GithubAppService, SandboxSessionService, cliIdentityResolver } from "@everdict/application-control";
...
export function buildSandboxSessions(opts: {
  ...
  const git: SandboxSessionServiceDeps["git"] =
    githubApp !== undefined
      ...
      : undefined;
  // WHO the CLI runs as. The use-case lives in @everdict/application-control (a store read, a consume gate and
  // secret resolution is not wiring); this only says which stores it reads. Absent pieces = identities are not
  // configured and the seam stays undefined, so a session runs exactly the way it did before.
  const resolveCliIdentity =
    capabilities !== undefined && scopedSecretsFor !== undefined
      ? cliIdentityResolver({ capabilities, scopedSecretsFor })
      : undefined;
