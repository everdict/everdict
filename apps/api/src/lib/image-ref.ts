...
// Common image-ref validation — allows both an externally hosted http(s) URL and a data:image base64 uploaded after resizing on the web.
// (Self-contained, no storage infra — a small image resized to 256px sits directly in the profile/workspace TEXT column.)
// Shared by the profile avatar and the workspace logo (dedup). field is the field name in error messages/data.
