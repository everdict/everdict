...
export function registerBillingRoutes(app: FastifyInstance, deps: ServerDeps): void {
  ...
  app.get("/usage", { schema: billingDocs.usage }, async (req, reply) => {
  ...
  app.get("/budget", { schema: billingDocs.getBudget }, async (req, reply) => {
  ...
  app.put("/budget", { schema: billingDocs.setBudget }, async (req, reply) => {
    if (!deps.budget) return reply.code(404).send({ code: "NOT_FOUND", message: "budget not configured" });
    const principal = await resolvePrincipal(req, reply, deps);
    if (!principal) return reply;
    try {
