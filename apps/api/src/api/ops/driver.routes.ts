...
// as live-cluster runtime control). Both families' ledger ids are scorecard rows, so ownership is checked
// against the scorecard store (a foreign workspace's id reads 404, no existence leak).
export function registerDriverOpsRoutes(app: FastifyInstance, deps: ServerDeps): void {
  const ownershipGuard = async (tenant: string, id: string): Promise<boolean> => {
  ...
  app.get<{ Params: { family: string; id: string } }>(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        if (!(await ownershipGuard(principal.workspace, req.params.id)))
        ...
        if (err instanceof z.ZodError)
          return reply.code(400).send({ code: "BAD_REQUEST", message: "family must be batch | score" });
