...
export function registerInviteRoutes(app: FastifyInstance, deps: ServerDeps): void {
  ...
  app.get("/invites", { schema: inviteDocs.list }, async (req, reply) => {
  ...
  app.post("/invites", { schema: inviteDocs.create }, async (req, reply) => {
    if (!deps.membershipService)
      return reply.code(404).send({ code: "NOT_FOUND", message: "membership service not configured" });
    const principal = await resolvePrincipal(req, reply, deps);
    if (!principal) return reply;
  ...
  app.delete<{ Params: { id: string } }>("/invites/:id", { schema: inviteDocs.revoke }, async (req, reply) => {
    if (!deps.membershipService)
      return reply.code(404).send({ code: "NOT_FOUND", message: "membership service not configured" });
    const principal = await resolvePrincipal(req, reply, deps);
    if (!principal) return reply;
  ...
  app.post("/invites/accept", { schema: inviteDocs.accept }, async (req, reply) => {
    if (!deps.membershipService)
      return reply.code(404).send({ code: "NOT_FOUND", message: "membership service not configured" });
    const principal = await resolvePrincipal(req, reply, deps);
    if (!principal) return reply;
