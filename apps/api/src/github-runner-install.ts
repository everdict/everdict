...
//  (1) GitHub Actions 셀프호스티드 러너(config.sh — CI 가 이미지 빌드 + Everdict 호출)
//  (2) Everdict 워크스페이스-공유 러너(everdict runner — self:ws:<id> 잡을 lease 해 평가 실행)
// 두 워커는 같은 호스트에 나란히 산다. Everdict 러너 토큰은 워크스페이스-공유 러너를 새로 페어링해 발급하고(1회 노출),
...
export interface GithubRunnerInstallInput {
  ...
  label: string; // Everdict 러너 표시 이름
  apiUrl: string; // 컨트롤플레인 base — `everdict runner --api-url`
  githubLabels?: string[]; // GH 러너 추가 라벨(항상 self-hosted + everdict-<id> 에 더해)
  capabilities?: string[]; // Everdict 러너 초기 capability 라벨(러너가 붙을 때 재프로브)
...
export interface GithubRunnerInstallResult {
  ...
  githubRunnerLabel: string; // "everdict-<id>" — 워크플로 runs-on 에 넣는 라벨
...
export async function installGithubWorkspaceRunner(
  ...
  // (2) Everdict 워크스페이스-공유 러너 페어링 — 평문 rnr_ 토큰은 스크립트에만 실린다(저장은 해시).
...
// 빌드 서버에서 실행 — GitHub Actions 러너(config.sh) + Everdict 러너를 백그라운드로 기동. 토큰은 이 스크립트에만.
...
# Everdict 가 생성한 셀프호스티드 러너 설치 스크립트 — 이 빌드 서버에 GitHub Actions 러너 + Everdict 러너를 함께 세웁니다.
# 토큰이 포함되어 있으니 공유하지 마세요(GitHub 등록 토큰은 단기, Everdict 토큰은 1회 노출).
set -euo pipefail
...
# 2) Everdict 러너 (self:ws 잡을 lease 해 평가 실행) — everdict CLI 가 설치돼 있어야 합니다.
#    (미설치 시: npm i -g @everdict/cli  또는 배포된 바이너리 사용)
nohup everdict runner --pair "${p.everdictRunnerToken}" --api-url "${p.apiUrl}" > /tmp/everdict-runner.log 2>&1 &

echo "✓ GitHub Actions 러너 + Everdict 러너(${p.runtimeTarget}) 기동 완료"
  ...
  eval:
    ...
    steps:
      ...
      - uses: everdict-ai/run-eval@v1
        with:
          runtime: ${runtimeTarget}                # Everdict 평가를 이 워크스페이스-공유 러너에서 실행
