...
describe("API — workspace integrations (GitHub App / Mattermost)", () => {
  ...
  it("트레이스 싱크(복수): 관리자는 이름으로 등록·해제, member 는 하니스별 선택, viewer 는 조회만(등록 403)", async () => {
    const h = { authorization: "Bearer x" };
    const { app } = server({ requireAuth: true, authenticator: roleAuth(["admin"]) });
    // Given/When: 관리자가 싱크 2개를 이름으로 등록한다(시크릿은 이름 참조만).
    const put = await app.inject({
      method: "PUT",
    ...
    // Then: 목록에 둘 다 + 하니스별 선택은 등록된 싱크만(없는 이름은 400).
    ...
    // 싱크 해제 → 목록에서 빠지고 dangling 선택도 정리.
    ...
    // viewer: 조회는 가능(harnesses:read), 등록은 403(settings:write).
    const viewer = server({ requireAuth: true, authenticator: roleAuth(["viewer"]) });
...
describe("API — workspace image registry (이미지 분류 기준 + push 발행)", () => {
  ...
  it("관리자는 이름으로 등록·해제(복수), 조회는 viewer 도 가능(harnesses:read) — imagePrefix 포함, 비밀 값 없음", async () => {
    const { app } = server({ requireAuth: true, authenticator: roleAuth(["admin"]) });
  ...
  it("push 자격증명: member 는 시크릿 값 발급(복수면 ?name= 필수), viewer 는 403, 미등록 404 · push 미구성 400", async () => {
    ...
    // 레지스트리가 2개(bare, ghcr)라 이름 생략은 400(어느 것인지 지정 강제).
    expect(
      (await admin.app.inject({ method: "POST", url: "/workspace/image-registries/push-credentials", headers: h }))
        .statusCode,
    ).toBe(400);
