...
import { type UsageMeter, inMemoryUsageMeter, usageDay } from "@everdict/domain";
...
// fails metering (meter-only). The day is stamped ONCE per record so the in-memory series and the durable rows agree
// across a midnight boundary. docs/architecture/usage-metering.md
export function persistentUsageMeter(store: UsageStore): UsageMeter & { hydrate(): Promise<void> } {
  ...
  const meter: UsageMeter = {
    record(tenant, source, model, cost, evaluations = 0, day = usageDay()) {
      mem.record(tenant, source, model, cost, evaluations, day);
      void store.record(tenant, source, model, day, cost, evaluations).catch(() => {}); // best-effort persist — never blocks
    },
  ...
  return {
    ...
    async hydrate() {
      for (const row of await store.all()) {
        mem.record(row.tenant, row.source, row.model, { usd: row.usd, tokens: row.tokens }, row.evaluations, row.day);
      }
    },
  };
}
