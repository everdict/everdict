...
describe("persistentUsageMeter", () => {
  ...
  it("hydrate restores in-memory usage (incl. per-model items) from the store at boot", async () => {
    const store = new InMemoryUsageStore();
    await store.record("acme", "harness", "opus", { usd: 0.4, tokens: 400 }, 3);
    await store.record("acme", "judge", "haiku", { usd: 0.02, tokens: 20 }, 0);
    const meter = persistentUsageMeter(store);
