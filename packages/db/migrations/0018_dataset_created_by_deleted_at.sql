...
ALTER TABLE assay_datasets ADD COLUMN IF NOT EXISTS created_by text;
ALTER TABLE assay_datasets ADD COLUMN IF NOT EXISTS deleted_at timestamptz;

-- 살아있는 버전 조회가 핫패스 → 부분 인덱스(삭제 안 된 행만).
CREATE INDEX IF NOT EXISTS assay_datasets_live_idx ON assay_datasets (tenant, id) WHERE deleted_at IS NULL;
