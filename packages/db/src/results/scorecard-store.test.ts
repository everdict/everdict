...
describe("PgScorecardStore", () => {
  ...
  it("create persists the verdict-policy stamp, gates and trace projection — a record's fields never silently vanish on insert", async () => {
    ...
    expect(calls[0]?.params?.[24]).toBe(6);
    expect(calls[0]?.params?.[25]).toBe(JSON.stringify(stamp));
    expect(calls[0]?.params?.[26]).toBe(JSON.stringify([gate]));
  });
  ...
  it("create → INSERT carries the analysis_ref column + value (so a ref set on a fresh record survives insert)", async () => {
    ...
    expect(calls[0]?.params?.[19]).toBe(ref); // positioned after manifest+requested+scorecard ($20), before sink_export
  });
