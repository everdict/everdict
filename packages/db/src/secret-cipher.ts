...
// KEK(키암호화키)는 환경(EVERDICT_SECRETS_KEY)에서 — 운영은 Vault/KMS 로 대체 권장(infra-deploy 규칙).
...
export function aesGcmCipher(key: Buffer): SecretCipher {
  if (key.length !== 32) throw new Error("EVERDICT_SECRETS_KEY 는 base64 디코딩 시 32바이트(AES-256)여야 합니다.");
  return {
    encrypt(plaintext) {
...
// EVERDICT_SECRETS_KEY(base64 32B) 가 있으면 cipher, 없으면 undefined → 호출부가 폴백(generatedCipher)을 선택.
...
// in-memory 에선 시크릿이 어차피 휘발이라 안전하다. Pg 영속 운영은 EVERDICT_SECRETS_KEY 로 키를 고정해야
