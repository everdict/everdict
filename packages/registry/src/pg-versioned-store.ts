...
export class PgVersionedStore<T extends { id: string; version: string }> {
  ...
  private async registerReturning(
    ...
    // ── THERE IS NO OWNERSHIP AUTHORITY ON THIS WRITE ─────────────────────────────────────────────────
    //
    // The entity belongs to the tenant it is registered under, and nothing transfers it. This signature once
    // carried an `opts.authority` bag it never read; a parameter that reads like an ownership check while
    // gating nothing is worse than none (rule `protocol`: an unreachable refusal is a claim that a window is
    // closed, with nothing able to test it), so it was deleted rather than implemented.
