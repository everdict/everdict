...
// Postgres 기반 테넌트-소유 metric SSOT. (tenant, id, version) 키. 테넌트 소유 우선, 없으면 _shared 폴백.
// 스키마: @assay/db/migrations/0014_create_metrics. PgJudgeRegistry 와 동일 구조.
export class PgMetricRegistry implements MetricRegistry {
  constructor(private readonly client: SqlClient) {}

  private async ownsId(tenant: string, id: string): Promise<boolean> {
    const r = await this.client.query("SELECT 1 FROM assay_metrics WHERE tenant = $1 AND id = $2 LIMIT 1", [
      tenant,
      id,
    ]);
  ...
  private async ownerVersions(owner: string, id: string): Promise<string[]> {
    const r = await this.client.query<{ version: string }>(
      "SELECT version FROM assay_metrics WHERE tenant = $1 AND id = $2",
      [owner, id],
    );
  ...
  async register(tenant: string, spec: MetricSpec): Promise<void> {
    const existing = await this.client.query<MetricRow>(
      "SELECT metric FROM assay_metrics WHERE tenant = $1 AND id = $2 AND version = $3",
      [tenant, spec.id, spec.version],
    );
    const row = existing.rows[0];
    if (row) {
      if (!specsEqual(row.metric, spec)) {
    ...
    await this.client.query(
      "INSERT INTO assay_metrics (tenant, id, version, metric, created_at) VALUES ($1, $2, $3, $4, now())",
  ...
  async has(tenant: string, id: string, version: string): Promise<boolean> {
    ...
    const r = await this.client.query("SELECT 1 FROM assay_metrics WHERE tenant = $1 AND id = $2 AND version = $3", [
      owner,
      id,
      version,
    ]);
  ...
  async get(tenant: string, id: string, ref = "latest"): Promise<MetricSpec> {
    ...
    const res = await this.client.query<MetricRow>(
      "SELECT metric FROM assay_metrics WHERE tenant = $1 AND id = $2 AND version = $3",
      [owner, id, version],
    );
    return MetricSpecSchema.parse((res.rows[0] as MetricRow).metric);
  }

  async list(tenant: string): Promise<Array<{ id: string; versions: string[]; owner: string }>> {
    const r = await this.client.query<{ id: string }>(
      "SELECT DISTINCT id FROM assay_metrics WHERE tenant = $1 OR tenant = $2 ORDER BY id",
