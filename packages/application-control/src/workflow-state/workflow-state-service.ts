...
export class WorkflowStateService {
  ...
  // Idempotent on the list path — a workspace that has no board yet gets one rather than an empty settings
  // screen.
  ...
  async create(input: CreateWorkflowStateInput): Promise<WorkflowStateRecord> {
    // `regressed` is not a state anyone can add: an issue reaches it only by falling from a resolution, never by
    ...
    const siblings = await this.ensureDefaults(input.tenant);
    if (siblings.some((state) => state.name.toLowerCase() === input.name.trim().toLowerCase()))
      throw new ConflictError("CONFLICT", { name: input.name }, "This workspace already has a state with that name.");
    const now = this.now();
    const record: WorkflowStateRecord = {
      ...
      status: input.status,
      color: input.color,
      // At the end of the board — a new column is the newest step, and where it belongs is a reorder.
  ...
  // a workspace that decides its "In QA" column is really review, not progress — which MOVES every issue in it, so
  ...
  async update(
    ...
    if (fields.name !== undefined) {
      const siblings = await this.deps.store.listByTenant(record.tenant);
      if (siblings.some((state) => state.id !== id && state.name.toLowerCase() === fields.name?.trim().toLowerCase()))
        throw new ConflictError(
          "CONFLICT",
          { team: record.tenant, name: fields.name },
          "This workspace already has a state with that name.",
        );
    }
    const now = this.now();
  ...
  async remove(tenant: string, id: string): Promise<void> {
    ...
    if (siblings.length <= 1)
      throw new ConflictError("CONFLICT", { tenant: record.tenant }, "A workspace keeps at least one workflow state.");
  ...
  async defaultFor(tenant: string, status: IssueStatus): Promise<WorkflowStateRecord | undefined> {
    const states = await this.ensureDefaults(tenant);
