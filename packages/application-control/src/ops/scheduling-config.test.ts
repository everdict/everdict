...
import { parseAutoscale, parseTenantCounts, parseTenantWeights } from "./scheduling-config.js";

describe("parseTenantCounts / parseTenantWeights (operator fairness dials)", () => {
  it("parses tenant=value pairs with a '*' default for unlisted tenants", () => {
    const map = parseTenantCounts("acme=8, beta=4 ,*=16", "EVERDICT_TENANT_QUOTAS");
  ...
  it("without a '*' default, unlisted tenants resolve to undefined (caller applies its own default)", () => {
    const map = parseTenantWeights("acme=3", "EVERDICT_TENANT_WEIGHTS");
    expect(map?.get("acme")).toBe(3);
    expect(map?.get("other")).toBeUndefined();
  });

  it("unset/empty env → undefined (the dial is simply off)", () => {
    expect(parseTenantCounts(undefined, "E")).toBeUndefined();
    expect(parseTenantCounts("  ", "E")).toBeUndefined();
  });

  it("malformed entries fail the boot loudly instead of running unfair for weeks", () => {
    expect(() => parseTenantCounts("acme=abc", "E")).toThrow(BadRequestError);
    expect(() => parseTenantCounts("acme", "E")).toThrow(BadRequestError);
    expect(() => parseTenantCounts("=5", "E")).toThrow(BadRequestError);
    expect(() => parseTenantCounts("acme=0", "E")).toThrow(BadRequestError); // zero quota = a typo, not a policy
  });
});
