...
describe("FsService.writeFile — the memory secret guard", () => {
  ...
  const writable = (): { fs: WorkspaceFs; written: string[] } => {
    ...
    const fs: WorkspaceFs = {
      ...
      async write(_tenant, path) {
        written.push(path);
        return { path, name: path, kind: "file" };
      },
      async mkdir() {
        throw new Error("unused");
      },
  ...
  it("refuses a credential-looking token in a memory file, naming what it found", async () => {
    const { fs } = writable();
    await expect(
      new FsService(fs).writeFile("acme", {
        path: "memory/github-setup.md",
        content: `Use ${fakeGithubToken} to clone.`,
      }),
    ).rejects.toThrow(/never store credentials.*GitHub token/);
    await expect(
      new FsService(fs).writeFile("acme", { path: "memory/keys.md", content: fakeWorkspaceKey }),
    ).rejects.toThrow(/workspace API key/);
  });

  it("guards ONLY memory/ — the same content elsewhere on the tree is a normal write", async () => {
    const { fs, written } = writable();
    await new FsService(fs).writeFile("acme", { path: "notes/github-setup.md", content: fakeGithubToken });
    expect(written).toEqual(["notes/github-setup.md"]);
  });

  it("lets ordinary memory prose through", async () => {
    const { fs, written } = writable();
    await new FsService(fs).writeFile("acme", {
      path: "memory/cadence.md",
      content: "The team ships the eval report every Friday. Reference the GitHub secret by NAME, not value.",
    });
    expect(written).toEqual(["memory/cadence.md"]);
  });
});
