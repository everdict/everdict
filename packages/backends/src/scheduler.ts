...
// 용량 인지 스케줄러: 백엔드 여유를 보고 자리 있는 곳에 배치하고, 없으면 큐잉한다.
// 슬롯이 비면(디스패치 settle) 자동으로 큐를 펌프한다. HOL 블로킹을 피하려 큐를 훑어
// "지금 배치 가능한" 잡을 먼저 보낸다(공정 큐/WFQ 는 다음 슬라이스). Dispatcher 호환(드롭인).
export class Scheduler {
  ...
  dispatch(job: AgentJob): Promise<CaseResult> {
    ...
    if (this.queue.length >= max) {
      return Promise.reject(
        new RateLimitError("RATE_LIMITED", { queueDepth: this.queue.length }, "스케줄러 큐가 가득 찼습니다."),
      );
    }
  ...
  // 관측용 스냅샷(테스트/모니터링).
  stats(): { queued: number; inFlight: Record<string, number> } {
    return { queued: this.queue.length, inFlight: Object.fromEntries(this.inFlight) };
  }
  ...
  private async pump(): Promise<void> {
    ...
    try {
      let placedAny = true;
      while (placedAny && this.queue.length > 0) {
        placedAny = false;
        ...
        for (let i = 0; i < this.queue.length; i++) {
          ...
          if (candidates.length === 0) continue; // 지금 자리 없음 → 다음 잡 시도(HOL 회피)
  ...
  private runOne(entry: QueueEntry, name: string): void {
    this.registry
      .get(name)
      .dispatch(entry.job)
