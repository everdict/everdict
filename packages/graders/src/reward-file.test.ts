...
describe("reward-file grader (the reward is a published file, not an exit code)", () => {
  // THE COUNTEREXAMPLE. Given a wrong answer, a Terminal-Bench verifier writes reward 0 and exits 0. Reading that
  // run by its exit code — which is what `tests-pass` does, and what the terminal-bench/terminal-bench dataset
  ...
  it("fails a run whose verifier published reward 0 while exiting 0 — the exit code says pass", async () => {
    ...
    const byExitCode = await new TestsPassGrader("bash /tests/test.sh").grade(ctx(verifierCompute({ publish })));
    expect(byExitCode.pass, "the defect: an exit-code reading of a Terminal-Bench verifier passes every case").toBe(
      true,
    );

    const scores = await new RewardFileGrader({}).grade(ctx(verifierCompute({ publish })));
  ...
  it("passes a run whose verifier published reward 1", async () => {
    const scores = await new RewardFileGrader({}).grade(
      ctx(verifierCompute({ publish: { "/logs/verifier/reward.txt": "1\n" } })),
    );
    const verdict = primary(scores);
  ...
  it("a continuous reward is a measurement, and the pass threshold decides the verdict", async () => {
    ...
    const strict = primary(await new RewardFileGrader({}).grade(ctx(verifierCompute({ publish }))));
    expect(isMeasured(strict) && strict.value).toBe(0.75);
    expect(isMeasured(strict) && strict.pass, "Terminal-Bench's own convention is reward == 1 for solved").toBe(false);

    const lenient = primary(
      await new RewardFileGrader({ passThreshold: 0.5 }).grade(ctx(verifierCompute({ publish }))),
    );
  ...
  it("namespaces reward keys so a task-authored key cannot land on a constitutional metric name", async () => {
    const scores = await new RewardFileGrader({}).grade(
      ctx(verifierCompute({ publish: { "/logs/verifier/reward.json": JSON.stringify({ reward: 1, state: 1 }) } })),
    );
  ...
  it("a multi-key reward with no `reward` key states no single verdict — unmeasured, not derived", async () => {
    const scores = await new RewardFileGrader({}).grade(
      ctx(
        verifierCompute({ publish: { "/logs/verifier/reward.json": JSON.stringify({ recall: 0.4, precision: 0.9 }) } }),
      ),
    );
    const verdict = primary(scores);
  ...
  describe("a verifier that published nothing is a GRADER failure, never a zero", () => {
    ...
    it("reports unmeasured when the reward file is empty or unparseable", async () => {
      for (const content of ["", "   \n", "not-a-number"]) {
        const scores = await new RewardFileGrader({}).grade(
          ctx(verifierCompute({ publish: { "/logs/verifier/reward.txt": content } })),
        );
    ...
    it("reports unmeasured when reward.json is not a {key: number} object", async () => {
      const scores = await new RewardFileGrader({}).grade(
        ctx(verifierCompute({ publish: { "/logs/verifier/reward.json": JSON.stringify({ reward: "pass" }) } })),
      );
  ...
  it("materializes the task's tests/ payload into the container before running the verifier", async () => {
    const compute = verifierCompute({ publish: { "/logs/verifier/reward.txt": "1" } });
  ...
  it("requires compute — a service/browser harness has none and must not silently score 0", async () => {
    const bare = ctx(verifierCompute({}));
    const { compute: _dropped, ...withoutCompute } = bare;
    await expect(new RewardFileGrader({}).grade(withoutCompute)).rejects.toThrow(/requires compute/);
  });
});
