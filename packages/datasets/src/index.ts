...
// 데이터셋 인제스트: 외부 벤치마크 포맷(WebVoyager jsonl / 임의 jsonl / csv) → 테넌트-소유 Assay `Dataset`(EvalCase[]).
// 멀티테넌트 SaaS 에서 유저가 자기 데이터셋을 쉽게 워크스페이스에 추가하려면, 자기 포맷을 EvalCase 로 변환하는
// 매핑 레이어가 필요하다(레지스트리는 Assay Dataset 스키마만 받으므로). 변환 결과는 DatasetRegistry.register(tenant) 로 등록.
