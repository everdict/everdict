...
// The registration guard above makes authors state their effects; reading them at invocation is what the
// declaration is FOR. The decision itself (`effectsRequireConsent`) lives in CONTRACTS beside the
// EffectContract schema (the isMeasured precedent): the agent-runtime kernel consults it on every tool call
// and depends on contracts only. Re-exported here for domain consumers.
export { effectsRequireConsent } from "@everdict/contracts";
