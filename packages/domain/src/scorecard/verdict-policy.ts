...
// ── policy identity ──────────────────────────────────────────────────────────────────────────────────
// Content digest over the canonical (key-sorted) JSON of the policy document. FNV-1a 64-bit — a stable,
// dependency-free identity stamp (this is provenance, not a security boundary).
...
export function verdictPolicyDigest(policy: VerdictPolicy): string {
  const text = canonicalize(policy);
