...
// ask) → complete (final answer in body) | failed. Only present when authorKind is "agent". An agent posting through
// a tool skips the lifecycle — it already has its answer, so it is born "complete".
...
export const CommentRecordSchema = z.object({
  ...
  // Agent-answer fields — absent on ordinary member comments. Two origins: a @everdict answer in the thread, and
  // an agent posting through the comment tool/route while carrying a member's credential (it declares itself in
  // the request, so the thread credits Everdict instead of that member).
