...
// event-plumbing.md E0 outbox). SEMANTIC DATA ONLY: no `message`, no `recipient` — "a fact happened" is the
// domain's concern; "what sentence a person reads" and "whose bell rings" are the application projector's
// (fact-projection.ts renders both at the stamping choke point). The payload must therefore carry every
// value a rendering needs (names, identifiers, titles — not just filterable ids): the projector reads the
// fact, never the aggregate. TypeScript's excess-property check on transition literals is the enforcement —
// a domain fact that tries to author copy fails to compile.
...
// The PROJECTED fact — a DomainFact plus the presentation the application layer attached (the rendered
// one-line message; `recipient` is push-targeting metadata, never persisted). Application-side emitters
// (adapter-backed state via PlatformEventEmitter) author this shape directly — they ARE the projection layer.
export const PlatformFactSchema = DomainFactSchema.extend({
