...
// fields (a skill has no transport, no write flag and no secrets — it is instructions the agent reads). Every entry is
// a Skill record this workspace OWNS: a store skill joins the library by being copied in, so there is no second
// channel to distinguish here.
export const AgentSkillEntrySchema = z.object({
  key: z.string().describe("Stable key (skill:<id>)"),
  ...
  version: z.string().optional().describe("The skill's last stamped version"),
