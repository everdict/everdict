...
| G4.9 | **The PROMPT was not a component the platform could name.** A `command` harness's declared instance-variation channels were `env`, `unsetEnv`, `params` and `resources`. `params` is substituted RAW into the shell command (`packages/harnesses/src/command.ts`, beside `{{task}}` which is `shq`-quoted) — the CLI-flag channel, by design, not a text channel — so a multi-line prompt could not live there. A campaign that evolved the prompt had to smuggle it through an env key the image's entrypoint happened to read, which is what the SpreadsheetBench wave did (`overrides.env.CC_SCAFFOLD`, nine rounds). Nothing declared that a harness HAS a prompt, so `diff_harness_versions` reported "an env key changed", attribution could not point at the prompt as a slot, and a delegate briefed to change the prompt had no named place to put it. | **closed 2026-09-04** — the template declares `promptChannel` (the delivery, which differs per CLI), the instance carries `overrides.prompt` (the text), and `resolveHarnessInstance` writes both the env key and the resolved spec's own `prompt` field from one value: the digest seals it and `diffHarnessSpecs` now reports `prompt`. A prompt override with no channel, or beside a hand-written env entry for that key, is refused. RED-first counterexample. An `agent` subject was always unaffected (`instructions` is first-class). |
| G4.10 | **TOOLS are a variable on the AGENT axis and a constant on the HARNESS axis.** ⚠️ The first version of this row said tools were declared for attribution only, and that was wrong for agents — corrected here rather than left standing. `AgentSpec` pins `capabilities: CapabilityRef[]` (`{source, id, version}`, immutable version pins into the Capability Store, kinds `mcp \| code \| skill`), turns a first-party default off with `disabledDefaults`, and hand-wires a server with `mcpServers`. A `code` capability carries its own `code`, its `parametersSchema` (shown to the model verbatim), its `description` and its `examples` — and `describeWithExamples` (`apps/agent/src/code-tools.ts`) assembles the model-facing description from the last two. So a tool's implementation AND the words the model reads are both fields of an immutable version, and swapping which version an agent pins is an `agent` campaign. What remains true is the HARNESS half: `owns: { tools }` on a topology service is an attribution declaration, and a `command` harness's tools live in its image, so there is no tool set a harness round can vary without a rebuild. | **narrowed** — the agent axis is covered; the harness half is **OPEN** and may well be correct as it stands (tools belong in the image), the point being that nothing says so. |
