...
- **Managed nomad/k8s**: the backend swaps the TASK image to `case.image`
  ...
  so the container must boot the everdict agent itself (read `EVERDICT_CASE_JOB`, run the case, print the
...
```dockerfile
FROM everdict-job-runner:slim AS agent          # (--agent-image overrides)
FROM <base-ref>
...
- **K8s**: an initContainer (`everdict-job-runner:slim`) copying the binary into an `emptyDir` shared volume +
