...
status: current
updated: 2026-09-15
anchors: [packages/datasets/src/terminal-bench.ts, packages/graders/src/reward-file.ts, apps/cli/src/tasks-prebuild.ts]
---
# Standard task-format on-ramp — bring an existing agent benchmark, run it managed

> Status: **M2 — all five slices landed**: the pure mapper, the ingestion edge, the source kind on both doors,
> the prebuild/push helper with image warnings on register, and the web wizard. SSOT for how Everdict ingests
> the *standard agent-benchmark task formats* (Terminal-Bench first) into its harness-agnostic `Dataset` model.
...
should point Everdict at them and get a defensible verdict, without re-authoring the benchmark. The row-based
dataset on-ramp (`packages/datasets`: HuggingFace / jsonl → `CaseMapping` → `EvalCase`) fits tabular QA/web
benchmarks but not the **directory/container** task formats that coding & terminal agents use. This is the
on-ramp for those.
...
depends only on `@everdict/contracts`, and is fully unit-tested; **parsing the source files (YAML/git) is a boundary
...
| verifier command (default `bash /tests/test.sh`) + the `tests/` bytes | `graders: [{ id: "reward-file", config: { cmd, files, … } }]` — the reward the verifier PUBLISHES, never its exit code (`packages/graders/src/reward-file.ts`). A v1-era set opts back in with `verdict: "exit-code"` → a `tests-pass` grader |
| `task.toml` `[environment]` cpus / memory / gpus / network mode | `resources` (cores → millicores) / `network` (`public` stays absent) |
...
The `imageTemplate` (e.g. `ghcr.io/acme/tb-tasks/{id}:v1`) keeps the recipe terse. `everdict tasks prebuild`
(slice 4) builds and pushes them.
...
2. **Ingestion edge** — `parseTerminalBenchTasks(text)` → `TerminalBenchTask[]` → `terminalBenchToDataset`.
   ...
   whatever walks the repository reads `task.yaml`/`task.toml` (it has a filesystem — `apps/cli/src/task-set.ts`), and this package takes
...
5. **Web** — ✅ the add-benchmark wizard's source picker has a third option ("Task set"). Choosing it takes
   ...
   there are no fields to assign. The recipe schema (`packages/datasets/src/spec.ts`) enforces the row half
   rather than trusting the UI: `mapping` is required for a row-mapped source, and optional — never read — for a
   task set.
