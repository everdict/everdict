...
- `record(tenant, source, model, cost, evaluations?)` — accumulate cost against a tenant + source (`harness` |
  `judge` | `agent`) + model, so usage is itemizable as a **(source × model)** matrix.
...
- `usage(tenant)` — totals + a per-source split + a per-`(source × model)` `items` breakdown. Synchronous (fast
  reads for `GET /usage`).
...
- `UsageStore.record(...)` — an **atomic per-(tenant, source, model) increment** (`ON CONFLICT DO UPDATE SET usd =
  usd + …`), so concurrent writes accumulate correctly. Table `everdict_usage` (mig 0051; the `model` dimension +
  the widened `(tenant, source, model)` key added in mig 0081, additive).
