...
> Companion to `../00-target-architecture.md` (§4 `domain/failure`, §9). Status: HISTORICAL — a review artifact from the re-architecture review, never updated after it landed.
...
| The classification survives every process boundary (sentinel, MCP lease reply, HTTP bridge) | **application** — agent always emits a sentinel (even on crash); runner submits a classified result, `fail_job` only for unparseable jobs | agent main tests; runner-loop tests; live "straight through the stages" scenario |
