...
> `../00-target-architecture.md` (§4 `domain/run`, §9). Status: HISTORICAL — a review artifact from the re-architecture review, never updated after it landed.
...
(now `packages/domain/src/run/run.ts`): guard methods are the SSOT for legality, transitions return store
...
| Adopt/redispatch legality | ONE owner already: `packages/domain/src/run/run.ts` (`canAdopt`/`canRedispatch`) — the model example of guards; `AdoptOutcome` tri-state (adopted/absent/unknown) decided in backend adapters (`packages/backends/src/orchestrators/{nomad,k8s}.ts`) | aggregate moves verbatim to `domain/run`; adopt-outcome semantics become a domain function the placement adapters call (00 §3-3) |
