...
| Installation-token scoping | minted per call, narrowed to repos+permissions, ~1h TTL, never stored; `tokenForRepo` = contents:read transient `CaseJob.repoToken` (`github-app-service.ts:236-244`) | unchanged; declare "tokens are transient" as a domain invariant with the adapter enforcing TTL-only handling |
