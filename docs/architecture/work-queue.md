...
## 정확성: 부팅 시 고아 회수
배치/run 은 컨트롤플레인 프로세스 안에서 in-process 로 track 된다(단일 프로세스 전제) — 재시작하면
이전 프로세스의 in-flight 레코드는 이어받을 주체가 없는 유령이 되어 큐가 영원히 '실행 중'을 보인다.
부팅 시 `recoverInterrupted`(`startup-recovery.ts`)가 queued/running 배치·자식·standalone run 을
**failed(INTERRUPTED)** 로 종결한다. 같은 DB 를 공유하는 컨트롤플레인이 둘이면 남의 in-flight 도
회수되므로 단일 컨트롤플레인 전제를 지킬 것.
