...
- `packages/contracts/src/harness/harness-spec.ts` — extend the `frontDoor` schema (knobs 1–4); `agent-job.ts` (knob 5 pin).
- `packages/contracts/src/harness/harness-template.ts` — mirror the `frontDoor` extension in `ServiceTemplateSpecSchema`.
