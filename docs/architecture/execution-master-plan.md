...
| **W3 원장과 게이트** | P3 (agent runs → `Run{kind:agent}`; session keeps transcript + `runId`; fleet = console filter; `causedBy` stamping) then P4 (one admission gate; delegated envelopes **meter+headroom**; fan-out guards; **cascade cancel**) + E1 (cursor consumers: notifications/MM/webhooks re-based; LISTEN/NOTIFY nudge; dead-letter + lag) | — (게이트는 워크플로가 아니다 — 안티 카탈로그) | safe agent-scale fan-out; E3 |
