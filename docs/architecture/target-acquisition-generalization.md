...
- `packages/contracts/src/harness/harness-spec.ts` — `TopologyTargetSchema.acquire` (discriminated union; absence = provision).
- `packages/contracts/src/harness/harness-template.ts` — mirror `acquire` in `TemplateServiceSchema`/the service template spec.
