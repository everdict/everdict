...
Sources were initially checked on **2026-09-10** and expanded against full primary sources through **2026-09-13**. The [detailed companion](evolution-papers/README.md) now contains all **60 standalone English explanations**, with complete Korean translations in the research reader. Each note covers principles, methods, major experiment families, setup, results, component tests, related work, limitations, and proposed Everdict changes as applicable. Resource notes distinguish guidance from experiments. Versioned links identify the inspected revisions; source-specific discrepancies and missing details are retained in the notes. No experiment was independently reproduced, and incompatible models, splits, budgets, or metrics are not combined into a leaderboard. Original repository observations below are pinned to `42f5c7abc724b2e914b4efee9666cf68794f6748`; the September 13 status update is checked at `d33dfd3866c9bb8556649659d95a8c8101e45535`.

Reading depth is explicit: **F** means a detailed primary-paper explanation is complete; **R** means an official non-paper resource and its available supporting material were analyzed. All 53 publications now have F notes and all seven other resources have R notes; no entry remains at abstract-only or landing-page screening. Coverage and unverified implementation details are stated individually. Priority means usefulness for Everdict’s next experiments: **Now**, **Next**, **Later**, or **Context**, rather than scientific quality.
...
an explicit reading depth, and an Everdict implication with its principal limitation.
...
| R04 | SkillCraft (2026a)[^5] | Next / F | Composition benchmark | Evaluate acquisition and reuse of executable tool compositions across tasks. Measure whether Everdict creates reusable capabilities and saves work, not merely whether a skill file was generated. |
...
| R06 | SkillRet (2026)[^7] | Next / F | Retrieval benchmark | Audited skill pools, query generation, retrieval comparisons, and an 80-task downstream study. Retrieval gains are stronger than the small reported downstream change, 65.5% to 65.8%; model, corpus, and cost conditions matter. |
| R07 | Gemma 4 Technical Report (2026)[^8] | Context / F | Model report | Documents an executor family used by WikiSkill. Pin model and serving identity when testing transfer; its model results are background, not evidence for a skill evolution method. |
| R08 | Gemini 3.5 Flash model card (2026)[^9] | Context / R | Model card | The official card and evaluation methodology were read, including inherited reporting limits. Pin model, harness, sampling, and context settings; provider-reported benchmark results do not establish a skill-evolution effect. |
| R09 | LiveMathematicianBench (2026)[^10] | Next / F | Reasoning benchmark | Recent-paper theorem questions and proof-sketch-aware evaluation address memorization and reasoning sensitivity. Useful as a transfer challenge, but every live release needs a frozen dataset identity for comparison. |
| R10 | AA-Omniscience (2025)[^11] | Next / F | Calibration benchmark | Jointly measures factual recall and uncertainty handling, including penalties for wrong answers. Add calibration and abstention to evolution objectives where relevant; higher task completion alone can hide worse reliability. |
| R11 | SoK: Agentic Skills (2026)[^12] | Context / F | Survey | Maps discovery, practice, distillation, storage, composition, evaluation, and update. Use its lifecycle to check coverage and distinguish language, code, and policy skills; a survey is not an optimizer comparison. |
...
| R13 | PagedAttention (2023)[^14] | Context / F | Serving infrastructure | Efficient KV-cache management and sharing improve serving throughput. Relevant when evolution cost is dominated by repeated rollouts; this is an execution optimization, not a learning mechanism. |
...
| R16 | SkillNet (2026)[^17] | Next / F | Skill infrastructure | Connects skill creation, relationships, evaluation, and reuse at scale. Reuse Everdict’s existing knowledge entries and versioned skills; catalogue size and descriptive quality scores do not themselves prove task gains. |
...
| R21 | SpreadsheetBench (2024)[^22] | Now / F | Artifact benchmark | Real spreadsheet tasks use multiple input workbooks per instruction to test robust solutions. Measure transferable artifact manipulation, including values and structure; avoid using only one visible example as the objective. |
| R22 | Terminal-Bench (2026)[^23] | Now / F | Execution benchmark | Difficult terminal tasks provide environments and executable verification. The existing Everdict importer makes it a practical pilot; task tests must remain independent of mutable harness code. |
...
| R25 | GDPval (2026)[^26] | Next / F | Work-product benchmark | Professional tasks evaluate deliverable quality across occupations. Useful for checking whether improvements reach realistic work products; grading and domain coverage differ from deterministic coding-task evaluation. |
| R26 | SealQA (2026)[^27] | Next / F | Search reasoning benchmark | Conflicting, noisy, or unhelpful search results stress evidence use. Test evolved retrieval and verification strategies under distraction; extra inference effort is not automatically useful reasoning. |
| R27 | Humanity’s Last Exam (2026)[^28] | Context / F | Academic benchmark | Expert-level closed-ended questions expose remaining capability and calibration gaps. A broad transfer probe, but a poor sole objective for a platform focused on interactive harness and artifact behavior. |
| R28 | Qwen3.5: Towards native multimodal agents (2026a)[^29] | Context / R | Model announcement and cards | Official 4B and 9B model cards supply architecture, configuration, evaluation, and deployment details beyond the limited landing page. Treat multimodal and serving choices as experimental inputs, not evidence for an evolution algorithm. |
| R29 | Qwen3.6-27B (2026b)[^30] | Context / R | Model announcement and card | The official 27B model card was analyzed for capability, evaluation conditions, and deployment assumptions. Match model and serving identities before transfer; separate provider benchmarks from independently measured agent improvement. |
...
| R31 | ALFWorld (2021)[^32] | Next / F | Interactive benchmark | Text environments align abstract planning with embodied tasks. Useful for repeated-action failures and reusable procedures; adding it to an Everdict pilot requires an adapter and pinned environment, not just an imported score. |
...
| R37 | Agent Skills for Large Language Models (2026)[^38] | Context / F | Survey | Reviews architecture, acquisition, deployment, and skill lifecycle governance. Helps classify candidate types and portability constraints; its broader survey claims are not separately reproduced or used as measured Everdict outcomes. |
...
| R39 | ReAct (2023)[^40] | Context / F | Agent architecture | Interleaves reasoning and environment actions to improve grounding and recovery. Provides a baseline execution pattern and trace structure; it does not itself maintain an evolution archive or learn reusable cross-task skills. |
...
| R41 | TextGrad (2025)[^42] | Now / F | Textual optimization | Propagates language feedback to components of a computation graph. Useful for mapping observed failure to editable components; an LLM explanation is proposed credit assignment, not proof that a component caused the error. |
...
|---|---|---|---|
| E01 | Reflexion (2023)[^48] | Now / F | Verbal feedback stored across attempts is the simplest memory baseline. Compare against curation before building a larger wiki; episode-level reflection is not automatically cross-task generalization. |
| E02 | Self-Refine (2023)[^49] | Context / F | Iteratively critique and revise an output with the same model. Useful as a candidate-local control, but improved output on one task is not evidence of a reusable evolved agent. |
| E03 | Voyager (2023)[^50] | Next / F | Combines automatic curriculum, executable skill storage, and reuse in Minecraft. Connect task discovery to durable capabilities; its environment-specific progress measures need an Everdict-specific analogue. |
| E04 | Automated Design of Agentic Systems (2024)[^51] | Now / F | Meta Agent Search writes new agent programs using an archive of earlier designs. Broaden candidates to workflows and tools; expressiveness of code does not establish efficient search over every program. |
...
| E10 | MAP-Elites (2015)[^57] | Next / F | Preserves high-quality solutions across chosen behavioral dimensions. Use to maintain capability specialists and cost alternatives; manually chosen archive dimensions can misrepresent meaningful diversity. |
| E11 | POET (2019)[^58] | Later / F | Co-evolves environments and their agents, transferring solutions between challenges. Inform development curricula while freezing external confirmation tasks; harder generated tasks alone do not prove broader capability. |
| E12 | FunSearch (2023/2024)[^59] | Next / F | Pairs language-generated programs with systematic evaluation to discover reusable algorithms. A precedent for executable skill search; the function space and evaluator define what improvement means. |
| E13 | DSPy (2023)[^60] | Next / F | Defines compositional language-model programs whose parameters can be compiled against a metric. Separate program structure from optimizable prompts and examples; compiler integration is an experiment, not a required new platform dependency. |
...
|---|---|---|
| EvoSkill, R02 | OfficeQA 60.6% → 67.9% in prose, but 68.1% in Table 1; SealQA 26.6% → 38.7% | The OfficeQA discrepancy is unresolved in v1. Splits, tolerance grids, and parent-selection descriptions also need care; [R02](evolution-papers/R02.md) preserves these rather than choosing a convenient number.[^3] |
| SkillsBench v4, R15 | Headline mean 33.9% → 50.5%, +16.6 points | 87 tasks, eight domains, 18 configurations. The paper calls this pass rate, but its fractional-reward definition differs from Table 16 fully solved rates of 31.3% → 47.7%. Skill-length group comparisons are observational across tasks, not a controlled shortening experiment.[^16] |
| AHE v4, R17 | Terminal-Bench 2 pass@1 69.7% → 77.0% after ten iterations | Search and reporting use the same 89 tasks. This is adaptation performance, not a fresh held-out estimate. Predicted regression recall is only 11.1%, so predictions cannot replace verification.[^18] |
| SkillRouter v5, R45 | 74.0% Hit@1; removing bodies loses 37.3–44.0 points across baselines | 75 core queries over 78,361-skill and 79,141-skill pools, averaged across correlated conditions. Hit@1 requires any labeled skill, not all needed skills. Some downstream configurations regress despite stronger retrieval.[^46] |

Further results materially constrain the proposed evolution loop:

- **Adaptive selection is not confirmation.** HarnessX reports pass@2 peaks on its adaptation tasks; Meta-Harness's terminal experiment and AHE also reuse the search population. Self-Harness's “held-out” partition participates in adaptive acceptance. ADAS uses test-ranked architectures for transfer selection. Each [detailed note](evolution-papers/README.md) specifies the experiment rather than assigning one blanket reliability label to a paper.
- **A validation gate can still overfit.** SkillOpt's selection curve improves while some test curves peak and decline. WikiSkill's positive four-benchmark ablation is not a matched-budget comparison of wiki topology. Neither establishes that an arbitrary gate or curated store prevents regression. See [R38](evolution-papers/R38.md) and [S00](evolution-papers/S00.md).
- **Memory and reflection can help selectively.** Reflexion's Python MBPP result declines; Self-Refine's math improvement is much smaller than its dialogue preference gap. ACE and ReasoningBank have specific streaming, judge, and extra-compute protocols. Their gains should not be pooled into one expected memory benefit. See [E01](evolution-papers/E01.md), [E02](evolution-papers/E02.md), [E08](evolution-papers/E08.md), and [E09](evolution-papers/E09.md).
- **Novel discoveries and reliable search are different claims.** FunSearch finds the 512-cap in 4/140 searches; its packing study instead supplies train/validation/test separation and repeated-run results. POET's generated terrain and fixed reward define a bounded curriculum, while its budget comparison does not charge the control for the entire discovery population. See [E12](evolution-papers/E12.md) and [E11](evolution-papers/E11.md).
- **Compilation can change both behavior and cost.** DSPy's strongest configurations combine demonstration selection, architecture, and ensembles; one HotPotQA result uses half the test subset. Its T5 result changes weights and is development-only. Preserve the compiled artifact and all resource costs. See [E13](evolution-papers/E13.md).
...
## Repository observations and current proposal status
...
Sources below were checked initially on 2026-09-10 and expanded through 2026-09-13. Preprint versions are pinned where available.
...
[^7]: Kang et al. (2026). *SkillRet: A Large-Scale Benchmark for Skill Retrieval in LLM Agents*. [Primary source](https://arxiv.org/abs/2605.05726v3).
