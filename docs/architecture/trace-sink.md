...
- **Scores exported = all MEASURED scores** (graders + `judge:<id>`), mapped to the platform's native
  ...
  `measuredScores` gate every internal aggregate uses: an unmeasured score has **no `value` at all** (the
  measurement algebra's non-measurement variants carry none), and inventing one would put a dead grader into the
  tenant's platform as a genuine scored 0 — indistinguishable there from an agent that really failed, and no longer
  ours to correct. Unmeasured scores are omitted (the payload has no non-score slot;
