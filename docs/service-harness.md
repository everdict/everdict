...
> **Superseded (slice 5b — `ab7e2d2`):** `docker` is **no longer a registerable `RuntimeSpec` kind** (kinds are
> now `local|nomad|k8s`, and `buildRuntimeBackend` no longer builds a `DockerBackend`). "Single docker host" is now
> absorbed by the **self-hosted runner**: container execution is a `docker` **capability** the runner self-advertises,
> and a `case.image` runs in a local container via `DockerDriver` there. `DockerDriver`/`DockerBackend` the classes
> remain and drive that path. See `docs/architecture/self-hosted-runtime-and-runners.md` + `docs/architecture/portable-harness-runtime.md`.
