---
kind: wiki
title: "The gate catalog — what each control refuses, and the incident behind it"
status: current
updated: 2026-09-15
anchors: [scripts/ci-local.mjs, scripts/hooks/gate-decision.mjs, scripts/check-controls-documented.mjs]
---
# The gate catalog — what each control refuses, and the incident behind it

Every control this repository runs, with the incident that produced it and the ways it has been wrong. This
page used to BE `.claude/rules/ci.md`: 714 lines injected on every file edit by a `**/*` glob, which is the
opposite of what a rule is for (skill `documenting`: a rule is thin and pushed; the reason is a record). The
rule now carries only what must not be forgotten at the keyboard, and this page carries the why — moved
verbatim, grouped by what each control guards. See `docs/architecture/repository-layout.md`.

How to use it: `pnpm ci:local` runs the gate (skill `ci` is the recipe); `pnpm triage <gate>` explains a red
one from the scanner's own header; this page is where to look when you need to know why a control exists
before changing it. `pnpm controls-documented` refuses a `package.json` control this page never names.

Where a bullet says "this file" or "three bullets down", it means the rule it was written in; the text is
kept as written, because the wording is part of what each incident recorded.

## The push gate, its ledgers, and the stamps a push needs

The hook, what it reads, and the three extra stamps (evals, review, release). The rule that pushes the short form is `.claude/rules/ci.md`.
...
- **Enforced, not advisory**: a PreToolUse hook (`scripts/hooks/pre-push-gate.mjs`, wired in
  ...
  .github/workflows directory is gone (declared as C3 on `docs/sdlc/declared-limits.md`). So there is nothing to watch after
...
- **The gate RECORDS what it decided** (`.git/everdict-gate-log.jsonl`, one JSON line per push decision).
  ...
  signal names in `scripts/telemetry/README.md`; see `docs/sdlc/observability.md`.
...
- **`pnpm review` is the review that had no moment, and `docs/sdlc/releases/<tag>.md` is the release that had no gate.**
  ...
  tag is denied unless `docs/sdlc/releases/<tag>.md` is COMMITTED — committed, because an authorization living in a
...
- **`pnpm agent-evals` is the configuration's own regression suite, and it is NOT in this gate.** `docs-check`
  ...
  that CHANGES `CLAUDE.md`/`.claude/**`/`docs/sdlc/gates.md`/`scripts/evals/**` unless `.git/everdict-evals-ok` stamps HEAD — tip-only,
  ...
  `scripts/evals/README.md`. ⚠️ Its first assertion was the literal `ci:local` and it went red
  against an answer that had RUN the gate and written "CI-local": assertions name artifacts the agent must
  reach for, never phrasing. See `scripts/evals/README.md`.
- **`pnpm fix-proof` reads the rule CLAUDE.md has carried since the first week — every fix ships a
  ...
  under `scripts/` and `scripts/evals/` are outside it — their proof is a truth table or a drill. Observed on two
...
## The change chain — intent, design, plan

What `docs/sdlc/intent/` is held to, and the clauses declared rather than enforced.
...
- **One clause is DECLINED rather than blocked** — an implementation with no `plan.md` is not refused, because
  `docs/sdlc/intent/README.md` says a one-line fix does not need one and a gate demanding otherwise gets a plan that
  says nothing. `docs/sdlc/declared-limits.md` keeps chosen limits in their own section with
...
- **Five L4 clauses cannot be satisfied by this deployment, and one is DECLINED — all DECLARED, not silently missing** —
  `docs/sdlc/declared-limits.md`. Branch protection needs a second person; managed settings
...
## Keeping the conventions and documents honest

Checks over `CLAUDE.md`, `.claude/**` and `docs/**` — the text that steers the agent and records why.
...
- ⚠️ **`docs-check` REQUIRED a citation form it then refused to read.** A link out of `docs/` must be an
  ...
  harness added for `docs/sdlc/intent/`, `docs/sdlc/releases/`, `docs/sdlc/lessons/` and `scripts/evals/` were exactly that class: the directories
...
- **`pnpm controls-documented` asks the question no other gate asks: does every control that exists get NAMED
  ...
  `docs/sdlc/lessons/2026-09-05-a-control-shipped-and-the-conventions-did-not-know.md`.
...
- **`pnpm lesson-evals` verifies the incident-to-eval route instead of trusting it.** The article's rule is
  that each production incident becomes a permanent eval; `docs/sdlc/lessons/README.md` says where a lesson goes
...
- **`pnpm doc-anchors` gives product documentation an owner: the change that makes it false.** `anchors:` had
  been in 106 documents' frontmatter, naming the files whose change should force a review of the page, and the
  only reader was `docs-check` asking whether each path EXISTS — a declaration with no reader. It showed: in the
  month before this check, 467 commits changed `packages/` or `apps/` and `docs/guide/` was edited three times,
  never with code. The rule, over the commits a push carries that are newer than the check: a changed file some
  document anchors, with that document unchanged in the same push, needs `Docs-unchanged: <doc> — <why>` in a
  commit body. Per DOCUMENT, because the value is that the author reads the list, and a blanket line is the
  answer that lets them not; per PUSH, because a page fixed in the last commit was fixed with the change.
  ⚠️ **Its cost is the anchors' breadth, and it was measured twice before it was wired.** Against the anchors
  as they stood, 314 of 601 code commits in a month touched an anchored file — `apps/api/src/main.ts` alone 67
  times. The audit that rewrote every product page narrowed each anchor to the file that DEFINES the documented
  surface, and the replay barely moved: 48.7% of commits, because the defining files are the central contract
  schemas (`eval-case.ts` changed 96 times). Split by kind, the answer was plain — pages people FOLLOW (the
  guide, the root reference pages, runbooks, migration preflights) would be owed on 23.7% of commits; the
  architecture design records on 43.9%. So the followed pages are ASKED, and `docs/architecture/**` is
  ADVISORY: printed as `· advisory:`, never refused, and read by the documentation pass in `REVIEW.md`. A gate
  that asks on nearly half of all commits gets a rote answer, and a rote answer is no answer. Superseded
  decisions, `docs/sdlc/` records and the frozen re-architecture folder are out of scope. The predicate lives in
  `scripts/doc-anchors.mjs` and is driven over a truth table first, every run.

## Scanners over product code

Each one is a law from rule `protocol` or a layer rule that failed as prose and was made mechanical. Most are ratchets over a baseline.
...
## Mutation and trust suites — what the gate deliberately does not run

The on-demand proofs, and the checks that make their absence visible.
...
## Readers of code nobody touched, and of the harness itself

Controls that are not change-scoped: the scan, the band watcher, and the readers of what the harness records.

- **`pnpm scan` is the only control here that is NOT change-scoped**, and `docs/sdlc/lessons/` is where an incident's
  ...
  found two real ones in 291 untouched files — see `docs/sdlc/intent/2026-09-05-scan-contracts-outbound-credentials/`.
- **`pnpm scan --dismiss --scope <s> --file <p> --reason <why>`**, and the record is COMMITTED
  (`docs/sdlc/scan-dismissals.md`). A dismissal is a decision; `.git/` does not travel, and a dismissal nobody else can
...
- **`pnpm watch-bands` is the only thing here that starts work without a person**, and `pnpm triage <gate>` is
  ...
  chain never returned to the queue on its own. The watcher reads `scripts/evals/history.jsonl` and
  ...
  `intent.md` into `docs/sdlc/intent/` and has no other route — `pnpm intent-chain` then applies to it exactly as to a
...
- **`pnpm telemetry-report` reads the sink back, and `pnpm findings` grades what the reviewer found.** Two
  ...
  whose findings nobody graded. `docs/sdlc/finding-dispositions.md` is COMMITTED (the reports are this checkout's
...
## Tooling traps and the permission surface
...
- **The permission surface has a deny half now, AND IT IS A SPEED BUMP, NOT A BOUNDARY.** Every session here
  ...
  shell. `docs/sdlc/declared-limits.md` §4 already declares those two containment-drill rows
