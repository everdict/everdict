...
The tracker is the missing layer. It is deliberately Linear-shaped, minus Teams — a workspace already *is* the
team boundary.
...
An issue gathers the capabilities that verify it (`links[]`: harness · dataset · judge · scorecard · run ·
...
Full BFF↔MCP parity. HTTP under `/issues`, `/projects`, `/initiatives`; the MCP twins are `create_issue`,
`list_issues`, `get_issue`, `update_issue`, `set_issue_status`, `add_issue_link`, `remove_issue_link`,
`list_issue_scorecards`, `delete_issue` plus the six-tool sets for projects and initiatives. The MCP surface is
how an agent triages its own regressions: find the issue watching a harness, read how it was closed last time,
move it.
...
packages/db/src/tracker/                       InMemory + Pg stores (migration 0103)
apps/api/src/api/{issue,project,initiative}/   routes + MCP + OpenAPI docs
```
