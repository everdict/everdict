...
| TRUST-69 | **Each release-CAS dimension falls back on its own** (real Postgres): a product with a populated policy digest and a still-NULL definition digest — what a rolling deploy of mig 0160 produces — still conflicts when the product moves, because a dimension with no digest is guarded by the row version rather than waved through | `apps/api/src/trust/release-readiness.trust.test.ts` |
...
| TRUST-119/120 | **One plan, consumed many times — never re-derived**: the facets a batch sealed (selection, harness document, model pins, judge closure, the re-score variant) are read through a single `ExecutionPlan`, and every execution path asks it instead of rebuilding it from the manifest — including the refusal a VANISHED sealed harness must produce, since `undefined` means both "built-in" and "gone" and only the seal tells them apart. The second half is structural and is the point: a source scan asserts each facet is read in exactly ONE file, over code lines only, because reconstruction is not a type error | `packages/application-control/src/scorecard/execution-plan.trust.test.ts` |

| TRUST-121 | **Every mutable input that composed a decision is that decision's read-set** (PRODUCTION seam, real Postgres): a ship CASes the open issues it counted, the candidate it compared, and the capability documents its contracts were resolved from — all in the same statement as the write, so an issue linked, a newer batch landing, or a registration shadowing a `_shared` document between the decision and the commit refuses instead of shipping a history entry that says `openIssues: 0`. What no row can carry (a soft delete, a settings edit) stays a labelled RE-VERIFY, because implying the stronger guarantee is worse than stating the weaker one | `apps/api/src/trust/release-decision-readset.trust.test.ts` |
