...
| TRUST-119/120 | **One plan, consumed many times — never re-derived**: the facets a batch sealed (selection, harness document, model pins, judge closure) are read through a single `ExecutionPlan`, and the four execution paths ask it instead of rebuilding it from the manifest. The second half is structural and is the point: a source scan asserts each facet is read in exactly ONE file, because reconstruction is not a type error — the last four waves each produced a field that existed and one consumer nobody taught about it | `packages/application-control/src/scorecard/execution-plan.trust.test.ts` |
