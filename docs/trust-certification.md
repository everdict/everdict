...
| TRUST-72/73 | **A shadowed registry document cannot execute under a held name**: a version is immutable inside ONE namespace and lookup is owner-first over `_shared`, so a workspace-local `support@1`/`agent@1` registered after submit hands every re-resolution (Temporal plan, resume, retry, re-score) different bytes under the same name — caught per case, per default-grader set, and per harness document, and REFUSED rather than re-sealed | `packages/application-control/src/scorecard/sealed-execution.trust.test.ts` |
| TRUST-78 | **Authority is stamped by a trusted boundary, never inferred from a producer-controlled label**: an undeclared grader emitting `state`/`tests_pass` becomes an invalid row instead of ground truth, a grader may not write into a judge's family (where a re-score could never replace it), and a producer HOLDING the declaration is untouched | `packages/application-execution/src/metric-authority.trust.test.ts` |
