...
| TRUST-160 | **A sealed recording is final, and sealing it is one decision** — the seal read the tracks and then wrote by run id alone (a reset in between let an attempt freeze a row it never read), and `append` never asked whether the row was sealed, which matters because the self-hosted lane reports fire-and-forget: a post-settle append is ordinary. Both conditions live in the write now, and a reset re-opens the buffer for the next attempt alone | `apps/api/src/trust/carried-authority.trust.test.ts` |
