...
| TRUST-121 | **Every mutable input that composed a decision is that decision's read-set** (PRODUCTION seam, real Postgres): a ship CASes the open issues it counted and the candidate it compared, in the same statement as the write — so an issue linked, or a newer batch landing, between the decision and the commit refuses instead of shipping a history entry that says `openIssues: 0`. The ambient half (registry/settings-resolved contracts) has no row to fence and is a labelled RE-VERIFY, not a CAS | `apps/api/src/trust/release-decision-readset.trust.test.ts` |
