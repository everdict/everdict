...
status: current
updated: 2026-09-15
anchors: [packages/contracts/src/execution/dataset.ts, packages/datasets/src/spec.ts, packages/datasets/src/catalog.ts, packages/datasets/src/travel.ts, packages/application-control/src/dataset/dataset-service.ts]
---
...
are comparable across harnesses/versions on identical cases (`Dataset` is distinct from `@everdict/contracts`' `Suite`, which
...
- **`_shared` benchmark tier** — first-party datasets readable/runnable by *every* tenant (owner-first,
  ...
  tier, and there is no directory loader — a `_shared` dataset is registered through the same doors as any other.
  The API boot line says so out loud — an empty `_shared` tier prints the door that fills
...
- **Role-gating** — `datasets:read` = viewer+, `datasets:write` = **member+** (datasets are collaborative
  eval *content*). `datasets:delete` = admin (the creator override lets the original author delete their own
  without being admin).
...
`CommentService` behind `GET/POST /comments` + `DELETE /comments/:id` (+ MCP `list/create/delete_comment`,
parity). `resourceType` is a free string, so the same comments serve other resources too. Role-gating: `comments:read` = viewer+,
...
- **Source wizard (primary)** — `POST /benchmarks/preview` fetches a few raw rows (no mapping) and returns the
  detected **fields** + samples; the web `/{workspace}/datasets/import` "create from source" wizard shows those rows as a
...
- **Catalog** — `GET /benchmarks` lists the first-party code catalog (`packages/datasets/src/catalog.ts`:
  webvoyager/gaia/swe-bench (lite + verified)/browsecomp/webarena/mind2web/gsm8k/osworld/tau-bench + the travel
  family below); `POST /benchmarks/import {benchmark}` pulls it. An entry may declare what a score from it IS
  (`scoring`: `official` names the evaluator it reproduces — gsm8k/gaia/swe-bench; `proxy` names what it
  approximates — tau-bench/browsecomp/webarena/the travel family, and a proxy number is an everdict-internal
  regression signal, never a leaderboard one). webvoyager/mind2web/osworld declare nothing, which reads as
  unstated, never as comparable. A Terminal-Bench task set imports separately (`POST /datasets/terminal-bench`,
  MCP `import_terminal_bench`). Mind the **env kind** each entry maps to: the
...
- **Recipe** — `POST /benchmark-recipes` saves a reusable `BenchmarkAdapterSpec` (`BenchmarkRegistry`, owner +
  `_shared`); `POST /benchmarks/import {recipe}` imports from it. The produced dataset records `producedBy` naming
  the recipe (and version) that made it. The web has no recipe pages (they were removed); the dataset detail's
  back-link still points at `/{workspace}/recipes/:id`.
...
carries `BenchmarkOrigin`; the wizard doesn't collect it) and, for a catalog entry, its **`scoring`** declaration. The web dataset detail renders these as a **Source · Lineage**
...
workspace secrets (admin-only) or env vars — registers `HF_TOKEN` themselves at Settings › Personal secrets and
imports a gated benchmark end-to-end from the web. The wizard is state-aware: gated + no token ⇒ warning + deep link to
`/{workspace}/settings/personal-secrets`; token present ⇒ "using the HF_TOKEN from my secrets / workspace secrets" (note: a member can't
...
`producedBy` (`{ via: recipe|catalog|spec, id, version?, source?, origin?, scoring? }`) is **content** stamped at
import (it *is* part of `specsEqual`) — the intrinsic "how it was made" and where the rows came from. `createdBy` and the
...
`DatasetRegistry` (port in `@everdict/application-control`) — register (with `createdBy`), get, versions, own
versions, list, `creatorOf` / `softDelete`, and version tags, mirroring the other versioned registries. All reads exclude tombstoned versions; `creatorOf` /
...
workspace-scoped reads, `datasets:write` gating, `409`/`CONFLICT` on the immutable violation. The capabilities —
register (stamps `createdBy`), validate, list, get, diff, delete one version (`DELETE /datasets/:id/versions/:version`),
bulk delete (`DELETE /datasets/:id`, body `{versions?}`; omit = all), version tags, Terminal-Bench import, and
the constitution attestation — are in the generated route reference (`/docs`) and `tools/list`.
...
## Web (`apps/web`)
- **Datasets `/{workspace}/datasets`** — the workspace's OWN datasets (shared first-party benchmarks enter through
  import, not this list). `GET /datasets` returns per-dataset **`DatasetListEntry`** metadata, and the page joins
  `GET /scorecards` to derive each dataset's **related harnesses** and `GET /members` to resolve `createdBy` → a
  human name, under the shared list-view controls (facets, grouping, ordering). The "import from source" and
  register CTAs are role-gated off `/me` (`datasets:write`).
- **Import `/{workspace}/datasets/import`** — the source wizard above.
- **Register dataset `/{workspace}/datasets/new`** — id/version/description + cases-JSON with a **validate (dry-run)**
  step, then register (`POST /datasets`).
- **Detail `/{workspace}/dataset/[id]`** — version switcher, cases, activity timeline + comments, the Source ·
  Lineage card, related harnesses, and the delete dialog (admin or the author); **new version**
  (`/{workspace}/dataset/[id]/new-version`) and **version diff** (`/{workspace}/dataset/[id]/diff`: added/removed
  cases, per-case field changes, dataset meta changes) sit beneath it.
