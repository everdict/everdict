...
status: current
updated: 2026-09-15
anchors: [apps/desktop/src/bridge.ts, apps/desktop/src/token-store.ts, apps/desktop/src/notification-watcher.ts, apps/web/src/features/manage-runners/ui/runners-manager.tsx, apps/cli/src/runner-command.ts]
---
...
[Releases](https://github.com/everdict/everdict/releases/latest); the web app links them from its
download page.

Three things it does that a browser tab cannot:

**Pair this machine in one click.** The Runtimes page's **Connect my machine** section shows "Connect
this device as a runner", and the app registers a runner and stores the `rnr_…` token encrypted with the
OS credential store — no token copied through a clipboard, and never handed back to the page.

**Keep running when the window is closed.** Closing the window hides it to the tray, so leased jobs
continue while you are doing something else. Quit from the tray menu.

**Raise native notifications** for what the web notification bell collects.
...
Your subscription pays for the tokens, the run is own-pays rather than metered to the workspace, and
the run records that it came from a self-hosted runner.
...
same page can offer one-click pairing in the app and a download link in the browser.

## Headless machines

The desktop app is for machines with a screen. For a CI box or a server, run the same runner from the
CLI with a pairing token:
