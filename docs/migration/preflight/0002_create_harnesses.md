...
**Change:** additive (expand). Creates `assay_harnesses` (harness version SSOT) with PK `(id, version)` + an
`id` index. No destructive operation → ships with the deploy.

**Preflight:** `preflight(client, "0002_create_harnesses.sql")`
- `OK_TO_APPLY` — not yet in `assay_schema_migrations`; safe (idempotent `CREATE TABLE IF NOT EXISTS`).
...
`scripts/live/pg-harness-registry.mjs`): a `HarnessSpec` round-trips through `assay_harnesses`; `(id, version)`
...
**Rollback (contract):** `DROP TABLE assay_harnesses;` — only after no code reads/writes it.
