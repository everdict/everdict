...
> **Status: table dropped.** `everdict_harnesses` had no reader or writer from the removal of the flat harness
> registry (`e4686b4f`, 2026-06-23) until `0217_drop_flat_harness_registry` dropped it — see
> [0002_create_harnesses](0002_create_harnesses.md) and
> [0217-drop-flat-harness-registry](0217-drop-flat-harness-registry.md). Tenant-scoped harness versions with the
> `_shared` fallback live in `everdict_harness_templates` / `everdict_harness_instances`
...
**Invariant:** none is pinned — nothing read the table. The same `(tenant, id, version)` immutability and
...
**Rollback (contract):** taken, and it superseded repointing the PK back to `(id, version)` — the whole table
went in `0217_drop_flat_harness_registry` (see 0002).
