...
- `packages/trace`        — pull a harness trace from OTel/MLflow → normalized `TraceEvent` (for service harnesses); `buildTraceSource(cfg)` factory (kind/endpoint/headers) powers pull-mode scorecard ingest. + **outbound `TraceSink`** (`buildTraceSink`: mlflow|langfuse|langsmith|phoenix) — export judged case trace+scores to the tenant's observability platform (create-or-attach per case). See `docs/architecture/trace-sink.md`.
