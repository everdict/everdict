...
- `packages/graders`      — scoring, fully separate from the harness (tests-pass / cost / steps / latency) + model judge (`JudgeGrader` + `modelJudge`/`anthropicComplete` — LLM/VLM verdict from a trace, transport injected). See `docs/judges.md`.
...
- `packages/registry`     — **versioned SSOT** (harnesses + datasets + judges): `(tenant, id, version) → HarnessSpec` / `→ Dataset` / `→ JudgeSpec` (immutable versions, semver `latest`, tenant-owned + `_shared` fallback; in-memory / file-GitOps / Postgres `Pg*Registry` — async interface); `ServiceTopologyBackend.specFor` wires to the harness registry. Datasets are **harness-agnostic**; **Agent Judges** are user-registered `model`(LLM/VLM call) | `harness`(delegate) specs. See `docs/registry.md` + `docs/datasets.md` + `docs/judges.md`.
...
- `apps/api`              — **multi-tenant control-plane HTTP surface** (Fastify): owns auth (`@assay/auth`: OIDC + API keys → `Principal`, role-gated routes, `GET /me`), tenant-owned harnesses (`POST/GET /harnesses`), tenant-owned + `_shared` datasets (`POST/GET /datasets`, harness-agnostic eval-case bundles), user-registered + `_shared` Agent Judges (`POST/GET /judges`, `model`(LLM/VLM call) | `harness`(delegate)), async batch evals (`POST/GET /scorecards`, dataset×harness → aggregated `Scorecard`+summary via `runSuite`, + selected judges applied to each trace → `judge:<id>` scores; model judges call the provider with the tenant's SecretStore key), `POST /internal/tenant-keys` ({workspace}), async `POST /runs`→run-id + workspace-scoped reads, `RunStore`/`ScorecardStore` (in-memory or Postgres via `DATABASE_URL`), and the agent-facing **MCP server** (`/mcp`, Streamable HTTP, OAuth-protected via Keycloak "login like Linear" + API keys, role-gated tools — full BFF↔MCP parity for runs/harnesses/datasets/judges/scorecards). See `docs/api.md` + `docs/auth.md` + `docs/mcp.md` + `docs/datasets.md` + `docs/judges.md` + `docs/scorecards.md` + `docs/tenancy.md`.
- `apps/web`              — **SaaS web** (Next.js 16 App Router, FSD, Tailwind v4 + shadcn Toss-style): Keycloak (Auth.js) user login; a **token courier** — forwards the Keycloak access token as `Bearer` to the control plane and gets `workspace`/roles from `GET /me` (role-gated UI; control plane enforces). Per-workspace dashboard (runs/harnesses/datasets/judges/scorecards). Self-contained eslint+prettier (excluded from root Biome). Pure HTTP client of `@assay/api` (no `@assay/*` deps). See `docs/web.md` + `docs/auth.md` + `docs/datasets.md` + `docs/judges.md` + `docs/scorecards.md`. (Humans→Keycloak; agents→API keys/MCP.)
