...
- **Authenticators, composed.** `githubActionsAuthenticator` (GitHub Actions OIDC federation — keyless CI) +
  ...
  `AuthContext{ workspaceHint }` (the `x-everdict-workspace` header) — the GitHub federation matches the verified
...
- **Secrets:** store only the **SHA-256 hash** of an API key (`@everdict/db`); plaintext is returned **once** at
...
- **Role mapping:** OIDC roles = `realm_access.roles ∩ everdict roles`, empty ⇒ `viewer`; workspace = `workspace`
...
- **Dev fallback** (`x-everdict-tenant` → admin) is allowed **only** when `EVERDICT_REQUIRE_AUTH` is unset. In any
  deployed config set `EVERDICT_REQUIRE_AUTH=1` so a missing bearer is 401.
- **Keycloak fixtures** (`deploy/keycloak/realm-everdict.json`) are imported via `--import-realm`. A user missing
