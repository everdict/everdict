...
- **Browse + inspect + the conversion overlay** (`BrowsableTraceSource extends TraceSource`, what
  ...
  best-effort — a wrong filter field/grammar 400s the whole listing (not a silent widen), so re-check those first if a
  real server rejects the list. `inspect(traceId, mapping)`
...
- **Evidence slots (finalAnswer/dom/screenshot + custom) + snapshot synthesis.** `SpanAttrMapping` also carries
  ...
  `string | {key, path?, pick?}` selectors — `path` reaches INSIDE a JSON attr value (object or JSON string;
  simple dot/bracket, NOT JSONPath), resolution is selector-major with `pick` last-wins per selector.
  `spansToEvidence` resolves them into `TraceEvidence` (incl. `custom` for the free-form
  `SpanAttrMapping.evidence` record — names are identifier-validated, non-reserved, and become the judge
  template's `{<name>}` placeholders via `CaseResult.evidence`→`GradeContext.evidence`→`JudgeInput.custom`).
  A screenshot value classifies as inline bytes (data-URI/bare-base64) or a ref; http(s) refs resolve to bytes
  best-effort, and a dom/custom value that IS a URL auto-fetches its real text (`fetchTextArtifact`, capped;
  finalAnswer never fetched). **Credentials travel same-origin only** (SSRF/credential-leak guard) — a miss
  keeps the ref/URL, NEVER fails the pull. The span-based sources expose `fetchDetailed(runId) →
  {events, evidence?}` (optional on the `TraceSource` contract; native kinds/fakes fall back to `fetch`) and
  `inspect` returns the same `evidence`; the extracted final answer is also appended as the trace's final
  assistant message (`withEvidenceEvents`, deduped). The pull-ingest path synthesizes
