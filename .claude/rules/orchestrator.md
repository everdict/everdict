...
- A workflow that fans out (`scorecardBatchWorkflow`, `scoreGroupWorkflow`) uses a **bounded** lane count (no
  unbounded `Promise.all`) — a large batch must not flood activity slots; the worker's `Scheduler` does
  fine-grained cluster capacity gating on top.
