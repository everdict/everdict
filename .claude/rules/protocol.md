...
2. `pnpm protocol-mutations --only <the rung>` neutralizes the new protocol in the production file and the
   ...
   silently stops being tested. ⚠️ No gate runs this (a full pass is ~90 minutes of real builds), so it is
   the AUTHOR who runs it. `--only` takes seconds; nothing checks that you did. A rung added without ever being driven is the
