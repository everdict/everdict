...
(Agent Judges). All follow the SAME rules below; datasets are harness-agnostic case bundles, judges are
user-registered `model`|`harness` specs. See `docs/registry.md` + `docs/datasets.md` + `docs/judges.md`.
...
- **Tenant ownership + `_shared` fallback.** Resolution is owner-first, then `SHARED_TENANT` (first-party).
  ...
  `HarnessRegistry` / `DatasetRegistry` / `JudgeRegistry` — add a new versioned entity by mirroring this, not a
  new model.
