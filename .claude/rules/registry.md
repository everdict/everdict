...
Versioned SSOT — `(tenant, id, version) → HarnessSpec` (harnesses) and `→ Dataset` (datasets). Both follow the
SAME rules below; datasets are harness-agnostic eval-case bundles. See `docs/registry.md` + `docs/datasets.md`.
...
- **Tenant ownership + `_shared` fallback.** Resolution is owner-first, then `SHARED_TENANT` (first-party).
  ...
  `HarnessRegistry` and `DatasetRegistry` — add a new versioned entity by mirroring this, not a new model.
