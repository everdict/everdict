...
## Checklist
1. Score is a **discriminated union on `status`** (`@everdict/contracts`), not a flat bag of optionals: `MeasuredScore{graderId, metric, value, pass?, label?, detail?}` | `UnmeasuredScore{graderId, metric, status:"unmeasured", reason, retryable, detail?}` | `InvalidScore{…, status:"invalid", reason:"contract_violation"}`. A grader reads trace + snapshot, never mutates. On a MEASUREMENT `value` is always a number; a **categorical** outcome (tier/string/enum — "gold" | "correct" | "timeout") sets `label` (with `value` as the ordering key: `1<2<3` for an ordered enum, `0` for an unordered one), which flips the metric to a **distribution** summary (see checklist 3). **Measurement status**: absent = `"measured"`; a grader failure/skip emits `status:"unmeasured"` + `reason` (closed vocab `UNMEASURED_REASONS`) + `retryable` (`safeGrade` → `grader_error`, judge-runner `skip()` → `missing_secret`/`unsupported`/…). **A non-measurement carries NO `value`** — illegal states (a "measured" row with a failure reason, an unmeasured row smuggling a number) are unrepresentable, and `.value`/`.pass` are unreachable until the `isMeasured` type predicate narrows (`measuredScores` for the array form). EVERY aggregate (mean/passRate/distribution/verdict/diff) filters through that gate — a dead grader is never a 0. **All legacy tolerance lives in ONE read-time normalizer** wired into `ScoreSchema` as a `z.preprocess` step, so pre-status rows (the `[grader-error]`/`skipped: ` detail sentinels) and old placeholder-zero rows are mapped onto the union at EVERY parse boundary (job-result sentinel, Pg record schemas, MCP inputs, script-grader stdout) by construction — the sentinel regex exists in exactly one place. Structural guards (`raw-scores-guard.test.ts` in domain · application-control · trace) pin that no new consumer bypasses the gate. Per-metric `unmeasured` tallies ride `MetricSummary`; `retryableUnmeasured(sc)` (domain) is the targeted re-score worklist.
