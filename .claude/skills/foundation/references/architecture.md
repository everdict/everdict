...
| `@everdict/drivers` | `Driver` impls — in-sandbox compute (Local/Docker) | contracts, domain |
| `@everdict/environments` | `Environment` impls — the world acted on (Repo/Prompt/OsUse) | contracts |
| `@everdict/harnesses` | `EvaluableHarness` impls — the agent under test | contracts, domain, trace |
| `@everdict/graders` | `Grader` impls — scoring | contracts, application-execution, llm |
| `@everdict/trace` | pull a harness trace from OTel/MLflow → `TraceEvent` (+ outbound `TraceSink`); re-exports `TraceSource`/`TraceSink` from contracts beside the impls | contracts, domain |
...
| `@everdict/backends` | `Backend` impls (+ `capacity()`) — placement (Local/Docker, Nomad, K8s) + `Router`/`Scheduler`/`BackendRegistry`; re-exports the `Dispatcher` port (application-control) beside `Backend` | contracts, domain, application-control, drivers, job-runner |
...
| `@everdict/orchestrator` | durable control plane (Temporal): Direct/Temporal orchestrators + worker | contracts, backends, job-runner (dev: domain, db) |
| `@everdict/topology` | service-topology harnesses: `ServiceTopologyBackend` + Nomad/K8s builders + env manager | contracts, domain, application-execution, application-control, backends, graders, trace |
| `@everdict/self-hosted-runner` | self-hosted runner core (MCP lease loop, resilient session, kind-branch execution) | contracts, domain, job-runner, topology, trace |
| `apps/cli` | dev control plane (`everdict run`, `everdict worker`, `everdict runner`) | contracts, domain, application-control, backends, datasets, job-runner, orchestrator, self-hosted-runner, topology |
| `apps/api` | multi-tenant control-plane HTTP (Fastify): auth + tenant-owned harnesses/datasets/judges/runtimes + async runs/scorecards + MCP | contracts, domain, application-control, auth, backends, datasets, db, drivers, graders, images, job-runner, llm, registry, storage, topology, trace |
...
| `@everdict/images` | adapters behind the `WorkspaceImages` port (managed image store): copy/mirror, provenance, in-memory + registry clients. A registry client is not object storage, which is why it is not in `storage` | contracts, domain, application-control |
...
| `@everdict/sdk` | the one-call developer surface over the control-plane HTTP API: a pure, zero-dependency HTTP client of the public routes that mirrors only the response fields it reads. **Nothing in the monorepo depends on it** | (none) |
| `@everdict/otel` | user-facing OTLP-door helpers, dependency-free: `EVERDICT_SEMCONV` + resource/exporter config builders for sending traces TO everdict (`POST /v1/traces`). Python = env-only recipe. **Nothing in the monorepo depends on it** | (none; dev: trace, for the drift guard) |
| `apps/agent` | Everdict's OWN agent, the reference owner runtime over `@everdict/agent-runtime`: workspace conversations, the tool surface (fs · code · runs · evals · integrations · skills · knowledge), activation/triggers, HITL approval, memory. It CONSUMES the trust harness and is not part of it | contracts, domain, application-control, agent-runtime, db, drivers, llm, registry |
...
## The eval loop (runs inside the job-runner)
```
...
into an isolated unit; the job-runner reconstructs the harness+graders from a registry
...
## Build status & order
Built (under today's package names): contracts/domain → drivers(Local) → environments(Repo) → harnesses(claude-code/scripted) →
graders(tests-pass/cost/steps/latency) → application-execution (`runCase`) → job-runner → backends(Local/Nomad) + Router/Registry →
