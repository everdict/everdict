...
- `registry` — versioned SSOT (harnesses + datasets + judges + runtimes): `(tenant, id, version)→HarnessSpec` / `→Dataset` / `→JudgeSpec` / `→RuntimeSpec` (immutable versions, semver `latest`, tenant-owned + `_shared` fallback, file/GitOps loader); backs `ServiceTopologyBackend.specFor`. Datasets = harness-agnostic case bundles; Agent Judges = `model`|`harness`; Runtimes = local|nomad|k8s execution infra (`docs/datasets.md`, `docs/judges.md`, `docs/runtimes.md`).
- `apps/cli` — dev control plane (`assay run`, `assay worker`). `apps/api` — multi-tenant HTTP surface (Fastify): async `POST /runs`/poll/webhook + `RunStore` + workspace-owned harnesses/datasets/judges + async batch evals (`POST /scorecards`, dataset×harness→`Scorecard`) + agent-facing MCP (full BFF↔MCP parity). Postgres via `DATABASE_URL`.
