...
- `registry` — versioned SSOT (harnesses + datasets + judges): `(tenant, id, version)→HarnessSpec` / `→Dataset` / `→JudgeSpec` (immutable versions, semver `latest`, tenant-owned + `_shared` fallback, file/GitOps loader); backs `ServiceTopologyBackend.specFor`. Datasets = harness-agnostic case bundles; Agent Judges = user-registered `model`|`harness` specs (`docs/datasets.md`, `docs/judges.md`).
