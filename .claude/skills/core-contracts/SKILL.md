---
name: core-contracts
description: The @everdict/contracts dependency root — interfaces, Zod schemas, and the AppError model that every other package builds on. No I/O, no SDKs. Use when editing packages/contracts (contracts/schemas/errors).
...
2. Add the `export * from "./<file>.js"` line to `packages/contracts/src/index.ts`.
...
6. Verify no forbidden import crept in: `contracts` may not touch domain/adapters/`application-*`/apps/SDKs.
...
spine contracts MUST be interfaces — many impls live in adapter packages, the interface lives in `contracts`.
...
- (`Backend`, the placement analog, is one more — but it lives in `@everdict/backends`, not `contracts`.)
`ComputeHandle` holds a real sandbox: callers always `dispose()` in a `finally` (contract, not impl).

## The spine, file by file
- **Harness** — `packages/contracts/src/harness/harness.ts`: `EvaluableHarness` + `RunContext`. Process-boundary, so
  the harness may be any language; its native output is normalized to `TraceEvent`.
- **Environment** — `packages/contracts/src/execution/environment.ts`: `EnvSpec` (repo|browser|prompt|os-use) and
...
- `packages/contracts/src/infra/trust-zone.ts` — `TrustZone` + `assertHardenedIsolation()`: untrusted tenants (arbitrary
