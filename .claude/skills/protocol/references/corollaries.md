# Corollaries — what the five laws turned into, one incident at a time

Moved verbatim from `.claude/rules/protocol.md` on 2026-09-15. The rule had grown to 1,231 lines injected on
every edit to the kernel and the control plane; the laws and the definition of done stayed there, and each
corollary below is listed in the rule by its heading. Read the whole section before designing against one —
the heading is the lesson, the body is how it was paid for and the shape that holds.

Where a section says "above", "this file" or "the top of the file", it means the rule it was written in.
