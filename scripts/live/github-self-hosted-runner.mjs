// 준(半)-라이브 헬퍼: 실제 GitHub 조직/레포에 셀프호스티드 러너를 세우기 위한 Assay 쪽 자동화.
// 이 스크립트는 컨트롤플레인 API 를 호출해 (1) 워크스페이스-공유 Assay 러너를 페어링하고 (2) 내 GitHub 연결로
...
// 인증:
//   ASSAY_TOKEN=<Keycloak JWT 또는 ak_… API 키>   (권장, 실제 배포)
//   또는 dev 폴백: 아무것도 없으면 x-assay-tenant:default (로컬 dev 전용 — 실 GitHub 연결은 없음)
// 입력(env):
//   ASSAY_API_URL   컨트롤플레인 base (기본 http://localhost:8787)
...
//   LABEL           (선택) Assay 러너 표시 이름
//
// 사용: ASSAY_TOKEN=… REPO=acme/app node scripts/live/github-self-hosted-runner.mjs
import process from "node:process";

const B = (process.env.ASSAY_API_URL ?? "http://localhost:8787").replace(/\/$/, "");
const token = process.env.ASSAY_TOKEN;
const headers = {
  "content-type": "application/json",
  ...(token ? { authorization: `Bearer ${token}` } : { "x-assay-tenant": "default" }),
};
...
// 2) github-install — Assay 워크스페이스-공유 러너 페어 + GitHub 등록 토큰 mint + 설치 스크립트 생성.
const body = {
...
console.log(`▶ Assay 러너 페어링: ${install.runner.id}  (runtime=${install.runtimeTarget})`);
...
console.log("  1. 위 설치 스크립트를 빌드 서버에서 실행 → GitHub Actions 러너 + Assay 러너가 함께 뜬다.");
...
console.log("  3. PR/머지 → GitHub Actions 발화 → CI 가 이미지 빌드 → 나란히 붙은 Assay 러너가 self:ws 평가 실행.");
console.log("  4. 스코어카드 origin 에 repo/sha 가 남고, 평가 결과가 PR 체크로 회신된다.");
console.log("\n✓ Assay 쪽 준비 완료. GitHub 쪽 종단 발화는 위 단계로 사용자 환경에서 검증하세요.");
process.exit(0);
