...
try {
  ...
  console.error('verdict:', JSON.stringify({ status: verdict.status, passRate: verdict.passRate, passAtK: verdict.passAtK, flakeRate: verdict.flakeRate }))
  ...
  const usageRes = await fetch(`${baseUrl}/usage`, { headers: { authorization: 'Bearer ak_e2e', 'x-everdict-workspace': 'acme' } })
