// 라이브(Nomad): 실제 aider 가 gpt-5.4-mini(workclaw LiteLLM)로 시드된 버그를 고치고 Assay 가 tests-pass 로
// 채점한다 — 단 실행은 **실제 Nomad alloc(도커 컨테이너)** 안에서. NomadBackend 가 assay-agent 이미지를 잡으로
// 띄우고, agent 가 선언형 command 하니스(코드 0)를 해석한다: (aider 사전설치) → aider 실행 → tests-pass.
//
// 준비: nomad agent 가동 + assay-agent:local 이미지 빌드(python + aider 사전설치 포함).
// 사용: NOMAD_ADDR=http://127.0.0.1:4646 ASSAY_AGENT_IMAGE=assay-agent:local \
//       OPENAI_API_KEY=<litellm key> ASSAY_MODEL=chatgpt/gpt-5.4-mini node scripts/live/aider-nomad.mjs
...
const job = {
  ...
  harnessSpec: {
    ...
    id: "aider",
    version: "0.74.0",
    // aider 는 assay-agent 이미지에 사전설치(PATH) → setup 비움. (특정 버전이 필요하면 setup 으로 pip 설치 가능.)
    setup: [],
    command:
  ...
  evalCase: {
    // 유니크 id — Nomad job ID(assay-<id>)가 매 실행 달라야 죽은 이전 alloc 과 충돌하지 않는다.
