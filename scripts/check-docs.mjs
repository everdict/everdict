...
for (const doc of docs) {
  ...
  for (const m of text.matchAll(/\]\(([^)\s#]+)(#[^)]*)?\)/g)) {
    ...
    // which is the shape this file exists to refuse. Found in review, on four index entries a batch added for
    // directories outside `docs/` — the directories it was trying to make discoverable, cited in the one form
    // nothing read.
    //
    ...
    //
    // A URL PINNED TO A COMMIT is a claim about THAT commit, not about today's tree. When a page is removed or
    // moved, a permalink to `blob/<sha>/<path>` stays exactly as true as the day it was written, and reading it
    // against the working tree would force a historical citation to be rewritten into one its author never made. So a hex ref is resolved at its own commit,
...
// A rule may not: it describes what to do RIGHT NOW in code that exists.
{
  ...
  // instead of reporting anything a reader could act on. It is the shape `scripts/check-import-cycles.mjs` records for madge
...
// So the layer this repository calls the whole point of `docs/` — "only a document says why, and what
