...
emit(
  [
    ...
    "```",
    "",
    head ? `Your checkout is at \`${head}\`.` : "This checkout's HEAD could not be read.",
    "",
    '⚠️ **The dangerous case is a deployment BEHIND you, and it is an ANCESTOR of your HEAD — so "is it a commit',
    'I have?" answers *yes* for exactly the case that bites.* Compare them like this:',
    "",
    "```",
    "git merge-base --is-ancestor <commit> HEAD   # exit 0 AND <commit> != HEAD  →  the deployment is BEHIND",
    "```",
    "",
    "Behind means the tools you are about to use are OLDER than your code: a defect you just fixed can still be",
    "live, and a tool you expect may be absent from the catalogue. **Say the deployment is behind — do not",
    "re-open the finding.** Measured 2026-09-18: four merged, gated, pushed fixes were invisible to a session,",
    "and two findings were nearly re-opened on that basis. If `commit` is NOT an ancestor, the deployment is on",
    "a different line than your checkout. `commit: null` means the image carries no stamp at all — neither",
    "agreement nor staleness, and nothing you can compare.",
    "",
    ...
    "```",
    "",
    "That read answers about the entities you NAME, and a task that names none gets nothing from it. The anchor",
    "above is the one this checkout can state for you — everything else you learn (the harness, the dataset,",
    "the campaign) is another anchor worth asking with.",
    "",
    "**When you cannot name one, search instead.** This is the read that needs no anchor, and it is what a",
    "session has at the moment it knows least:",
    "",
    "```",
    '  search_files { pattern: "<what you are about to re-derive>" }   # case-insensitive, over the whole tree',
    '  search_files { glob: "docs/specs/**" }                          # what has been specified',
    "```",
    "",
    "It reaches the specs, plans and wiki the work produced (`docs/`), and every knowledge entry's BODY, which",
    'is mirrored to `knowledge/<id>.md`. So "I could not name an entity" is not a reason to start from nothing —',
    "it is the reason this read exists. ⚠️ `truncated: true` means the answer is a FLOOR: narrow and ask again",
    "rather than reading it as all there is.",
    ...(issueHint !== undefined
      ? [
          "",
    ...
    "",
    "The assembly files what it ANSWERED by itself — its path comes back as `receipt.path`. Account for the",
    "other half, the one only you know, with the tool that checks your citations:",
    "",
    "```",
    '  record_retrieval_use  { assembly_path: "<receipt.path>", used: [id…], outcome: "<one line>" }',
    "```",
    "",
